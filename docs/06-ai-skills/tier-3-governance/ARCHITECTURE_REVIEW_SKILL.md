...
Architecture Context:
  ...
  Multi-tenant: Yes (must have tenant_id)
```
...
const StrategySchema = z.object({
  id: z.string().uuid(),
  tenant_id: z.string().uuid(),  // Multi-tenant!
  name: z.string(),
...
const SignalSchema = z.object({
  id: z.string().uuid(),
  tenant_id: z.string().uuid(),  // REQUIRED for multi-tenant
...
- [ ] tenant_id is required in all multi-tenant schemas
...
```typescript
// ✅ CORRECT: Every query filters by tenant_id
const getUserStrategies = async (userId: string, tenantId: string) => {
...
const updateStrategy = async (strategyId: string, tenantId: string) => {
  return db.strategy.update({
    where: { id: strategyId },
    data: { updated_at: new Date() }
  });
  // ❌ ERROR: No tenant_id check! Could leak data from other tenants
};
...
// ✅ CORRECT: With RLS
const getAllStrategies = async (tenantId: string) => {
  return db.strategy.findMany({
...
- [ ] Every API endpoint accepts tenant_id (from auth)
- [ ] DB schema has `tenant_id` NOT NULL on all tables
- [ ] RLS policy enforced at DB level (PostgreSQL policy if available)
- [ ] Find violations: `grep -r "findMany()" src/ | grep -v tenant_id`
...
⚠️ ISSUE FOUND - tenant_id missing in one query
...
2. **RLS exposure** (Missing tenant_id in query)
   - Severity: CRITICAL
   - Resolution: 15 min
...
- [ ] Validation in place
- [ ] Tenant_id in all multi-tenant schemas
...
**Root Cause**: Multi-tenant query missing WHERE tenant_id filter
