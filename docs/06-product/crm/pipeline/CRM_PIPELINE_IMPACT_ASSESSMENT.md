...
| RLS                  | required       | Per-verb organization/workspace/contact/lead/stage scope; `brand_id` is not an independent security boundary        | pgTAP and cross-tenant tests                |
...
- Brand identifiers remain organization-scoped CRM data; no separate brand RLS isolation is permitted.
