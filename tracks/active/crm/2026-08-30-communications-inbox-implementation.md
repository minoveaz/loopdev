...
Deliver the first usable CRM Communications Inbox experience on top of the approved Communications Core boundary. The initial slice is deterministic and mock-backed so the UX, accessibility and responsive composition can be validated without Docker or Supabase.
...
- Real provider calls, webhook changes, migrations, RLS or server authorization changes.
...
- [ ] Accessibility and responsive behavior checks before visual review.
...
- [ ] Real API handoff and remaining Docker/Supabase gates are documented.
...
- **Starting commit:** `d993dfa`.
- **State reached:** Branch synchronized with merged documentation; implementation track created; first mock-backed Inbox route, SplitWorkspace wiring, deterministic fixtures and interaction test added.
- **Decisions, blockers and risks:** Mock-backed UI is authorized; Core persistence and provider validation remain blocked by Docker/Supabase availability.
- **Validation executed:** Track validator, registry check, ESLint and editor diagnostics pass. TypeScript has no errors in the Inbox slice but the app-wide command still reports unrelated pre-existing errors. Vitest is blocked by the current Windows dependency installation resolving Radix `aria-hidden` to an unavailable absolute path.
- **Next concrete action:** Run focused technical tests after the dependency resolution issue is repaired, then request visual review before Playwright.
- **Current integration:** `GET /api/communications/inbox` reads conversations, CRM contacts, channels and messages through the authenticated Supabase server client. `supabase/seed_communications_inbox.sql` provides one idempotent development conversation for `estar-protegidos`.
- **Current limitation:** Frontend mutations remain fixture-local until authorized POST/PATCH adapters and provider policy execution are implemented. Real reads require Communications Core migrations, an active CRM workspace and organization membership in the connected Supabase project.
