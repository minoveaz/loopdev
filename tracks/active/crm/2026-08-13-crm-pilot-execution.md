...
remoto. La implementación frontend de Leads queda autorizada el 2026-08-22 en la rama
`feature/leads-frontend-implementation`, con `CRM_LEADS_UI_IMPLEMENTATION_PLAN.md` como plan
autoritativo. La Fase 0 de readiness UI quedó completada con `CRM_LEADS_UI_CONTRACT.md`; Contacts
permanece fuera de alcance. Pipeline (#85) ya completó su delivery backend. Se autoriza ahora la implementación
...
### Evidencia focalizada de Leads UI — Fase 1

| Fecha      | Validación                                                            | Resultado                                                                                                                                                                                                                                                                                                                                                                                                                                                | Referencia                                                                                                                      |
| ---------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-24 | Lista Leads sobre `SplitWorkspace`                                    | Implementada en local: adapter GET tipado y validado, view model, búsqueda debounce, filtros de contrato, cursor pagination, tabla/cards responsive, estados loading/empty/filtered-empty/error/forbidden y panel contextual por renderer de `SuiteRuntime`                                                                                                                                                                                              | `apps/loopdev-os/src/suites/sales-crm/leads/`, `apps/loopdev-os/src/app/sales-crm/leads/`                                       |
| 2026-08-24 | Corrección del renderer de `ModuleContextPanel` en `SalesCrmShell`    | El renderer de `leads` devolvía siempre `<LeadContextPanel/>`, un elemento verdadero aunque el componente renderizara `null` sin Lead seleccionado; `SuiteCanvas`/`ModuleContextPanel` mostraban entonces un aside vacío con cabecera y botón cerrar en cada visita a `/sales-crm/leads`. Se corrige devolviendo `null` explícito cuando no hay `selectedLead`, cumpliendo el estado ya declarado ("panel contextual cerrado hasta seleccionar un Lead") | `apps/loopdev-os/src/suites/sales-crm/SalesCrmShell.tsx`                                                                        |
| 2026-08-24 | Consolidación de pruebas duplicadas y cobertura de `LeadContextPanel` | `lead-ui.test.ts` duplicaba casi integramente la cobertura de `leads.test.ts` (mapper y `getLeads`); se fusiona la cobertura única (labels de estado/origen, filtro `source`, respuesta inválida 502) en `leads.test.ts` y se elimina el archivo redundante. Se añaden pruebas focalizadas de `LeadContextPanel` (estado vacío, resumen del Lead, cierre, navegación a ficha/Contacto, acción de conversión deshabilitada, axe)                          | `apps/loopdev-os/src/app/sales-crm/leads/leads.test.ts`, `apps/loopdev-os/src/app/sales-crm/leads/leads-components.test.tsx`    |
| 2026-08-24 | Tests y gates técnicos                                                | Vitest focalizado de Leads 20/20 (antes 12/12), suite completa `loopdev-os` 191/191, typecheck de `@loopdev/ui` y `loopdev-os`, eslint focalizado, shell 39/39, registry/source-contract y `git diff --check` correctos                                                                                                                                                                                                                                  | `leads.test.ts`, `leads-components.test.tsx`, `pnpm test:shell`, `pnpm registries:check`, `pnpm certification:source-contracts` |

### UI/UX certification: Leads list Fase 1

- **Implementation:** `apps/loopdev-os/src/suites/sales-crm/leads/`.
- **Owner:** `sales-crm`; `SuiteRuntime` conserva el ownership del `ModuleContextPanel`.
- **Status:** `in-progress`; la revisión visual/Playwright y el cierre UAT quedan pendientes.
- **Contract:** verificado contra `CRM_LEADS_UI_CONTRACT.md` y `CRM_LEADS_UX_SPEC.md`.
- **Accessibility/Interaction/Responsive/States:** tests focalizados verificados, incluido `LeadContextPanel` (vacío, seleccionado, cierre, navegación, axe); evidencia browser pendiente.
- **Consumer/API ownership:** verificado; no se modifica Contacts ni `@loopdev/ui`.
- **Evidence:** `apps/loopdev-os/src/suites/sales-crm/leads/UI_UX_SPEC.md`, registry frontend y tests focalizados.
- **Reopen triggers:** nuevo consumidor, estado, acción, theme, recipe o cambio de ownership.

### Evidencia focalizada de Leads UI — Fase 2

| Fecha      | Validación                           | Resultado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Referencia                                                                                                                                                                         |
| ---------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-24 | Captura L1/L2 e idempotencia visible | Implementación técnica completa: `QuickLeadCapture` y `/sales-crm/leads/new` comparten `LeadForm`; se validan contacto existente/nuevo, origen, asignación automática o UUID autorizado, interés/producto, provider, externalId, campaña, UTM `medium/content/term` y nota inicial. El Contact nuevo reutiliza el contrato existente sin modificar Contacts. Reintentos con el mismo `source + externalId` muestran `reused: true`; la nota usa `POST /api/crm/notes` con clave determinista. | `apps/loopdev-os/src/suites/sales-crm/leads/`, `apps/loopdev-os/src/app/sales-crm/leads/`                                                                                          |
| 2026-08-24 | Tests y validación técnica           | Correcta: tests focalizados de Leads 58/58, suite completa, typecheck completo, lint completo, shell 39/39, registry, source-contract, ownership, links, format, `git diff --check` y track validator pasan. Revisión visual y Playwright se mantienen como gate final explícito.                                                                                                                                                                                                             | `leadCaptureForm.test.ts`, `QuickLeadCapture.test.tsx`, `LeadCaptureWorkspace.test.tsx`, `lead-capture-api.test.ts`, `pnpm test`, `pnpm typecheck`, `pnpm lint`, `pnpm test:shell` |

**Estado formal de Fase 2 UI:** implementación técnica completada; no se marca certificada hasta
revisión visual explícita y Playwright. El backend actual no persiste `utm_source` ni ofrece catálogo
de usuarios; no se inventan endpoints, por lo que esos puntos quedan diferidos y documentados.

### Evidencia focalizada de Leads UI — Fase 3

| Fecha      | Validación                 | Resultado                                                                                                                                                                                                                                                                                                                                    | Referencia                                                                                                                         |
| ---------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-24 | Previsualización y detalle | `LeadRecordPreview` compone estado, atribución, Contacto, Opportunities y Activity usando el detalle de Leads y el Customer 360 autorizado. El panel contextual conserva scroll independiente y enfoca el encabezado al cambiar la selección. `/sales-crm/leads/:leadId` resuelve `workspace`/`RecordWorkspace`; mobile navega al workspace. | `apps/loopdev-os/src/suites/sales-crm/leads/LeadRecordPreview.tsx`, `LeadRecordView.tsx`                                           |
| 2026-08-24 | Permisos y concurrencia    | Edición, reasignación y estados manuales requieren `crm.manage`; `crm.read` mantiene lectura. El backend no expone `expectedVersion` numérico ni catálogo de usuarios, por lo que se conserva `expectedUpdatedAt` y UUID. Un 409 marca `stale` y exige refresh explícito.                                                                    | `apps/loopdev-os/src/suites/sales-crm/leads/api.ts`, `CRM_LEADS_UI_CONTRACT.md`                                                    |
| 2026-08-24 | Tests focalizados          | API de detalle, rutas, mapper/panel, estados y navegación verificados con Vitest; typecheck de `loopdev-os` correcto. Playwright y revisión visual permanecen pendientes como gate final.                                                                                                                                                    | `apps/loopdev-os/src/app/api/crm/leads/[leadId]/route.test.ts`, `apps/loopdev-os/src/app/sales-crm/leads/`, `shellRouting.test.ts` |

**Estado formal de Fase 3 UI:** implementación técnica completada; no se marca certificada hasta
revisión visual explícita y Playwright. No se modificó Contacts ni se inventó una entidad paralela.

### Evidencia focalizada de Leads UI — Fase 4

| Fecha      | Validación                 | Resultado                                                                                                                                                                                                          | Referencia                                                                                           |
| ---------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
| 2026-08-24 | Cualificación y conversión | `QualifiedLeadGuard` exige `crm.manage` y estado `cualificado`; `CreateOpportunityFromLead` exige producto/interés y bloquea `contactId` heredado. Se consume el endpoint/RPC existente, sin endpoints inventados. | `apps/loopdev-os/src/suites/sales-crm/leads/QualifiedLeadGuard.tsx`, `CreateOpportunityFromLead.tsx` |
| 2026-08-24 | Idempotencia y refresh     | 201/200 se muestran como Opportunity creada/existente; 409 como conflicto con reintento. Tras éxito se refrescan Lead, Opportunities relacionadas y Contact 360.                                                   | `apps/loopdev-os/src/suites/sales-crm/leads/OpportunityResultPanel.tsx`, `api.ts`                    |
| 2026-08-24 | Tests y contratos          | Tests focalizados de UI/API/contracts y Axe pasan; typecheck, ownership, source-contract, registries, links y track validator pasan.                                                                               | `lead-conversion.test.tsx`, `lead-record.test.tsx`, `conversion/route.test.ts`, `crm.test.ts`        |

**Estado formal de Fase 4 UI:** implementación técnica completada. La revisión
visual y Playwright están bloqueadas hasta aprobación visual explícita del
usuario; la UI/UX no se marca certificada. Contacts permanece fuera de alcance.
...
### Evidencia focalizada de Leads UI — Fase 5

| Fecha      | Validación                           | Resultado                                                                                                                                                                                                                                                                                                                                      | Referencia                                                                                                                                                                                                                                             |
| ---------- | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-08-24 | Unit/integration Leads               | `99/99` tests focalizados pasan para view models, adapters, mutaciones, permisos, conflictos, idempotencia, asignación y envelopes de error; la suite completa serial pasa `862/862`.                                                                                                                                                          | `apps/loopdev-os/src/app/sales-crm/leads/`, `apps/loopdev-os/src/app/api/crm/leads/`, `apps/loopdev-os/src/services/crm/capture.test.ts`, `pnpm test -- --maxWorkers=1`                                                                                |
| 2026-08-24 | Accesibilidad e interacción          | Axe y tests focalizados cubren tabla, dialog/sheet, foco, teclado, navegación y mensajes ARIA en captura, detalle y conversión.                                                                                                                                                                                                                | `leads-components.test.tsx`, `QuickLeadCapture.test.tsx`, `LeadCaptureWorkspace.test.tsx`, `lead-record.test.tsx`, `lead-conversion.test.tsx`                                                                                                          |
| 2026-08-24 | Typecheck, lint, build y shell       | Typecheck, lint y shell `39/39` pasan. El build y `validate:ci` quedan bloqueados durante el prerender por `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_ANON_KEY` ausentes.                                                                                                                                                              | `pnpm typecheck`, `pnpm lint`, `pnpm test:shell`, `pnpm build`, `pnpm validate:ci`                                                                                                                                                                     |
| 2026-08-24 | Registry/governance/source contracts | Registry, source-contracts, ownership, links, Supabase governance, track validator, frontend quality gate y `git diff --check` pasan. `validate:changed` alcanza los controles seleccionados y se detiene en el build por variables Supabase ausentes. `docs:inventory:check` sigue desactualizado por el inventario generado del repositorio. | `pnpm registries:check`, `pnpm certification:source-contracts`, `pnpm contracts:ownership:check`, `pnpm docs:links:check`, `pnpm validate:supabase-governance`, `node scripts/tracks/validate-tracks.mjs`, `pnpm front:check`, `pnpm validate:changed` |
| 2026-08-24 | Tenant isolation y RLS               | El servicio de Leads valida asignaciones contra membresías activas y la migración añade FK/RLS compuestos; el test pgTAP focalizado no pudo ejecutarse por conexión rechazada a `127.0.0.1:54322` y Docker ausente. La evidencia local previa de `005_crm_security.sql` se conserva, sin reclamar nueva ejecución.                             | `apps/loopdev-os/src/services/crm/leads.ts`, `supabase/migrations/20260907000000_crm_lead_assignment_scope.sql`, `supabase/tests/database/005_crm_security.sql`                                                                                        |
| 2026-08-24 | Staging readiness y UAT técnico      | `NOT READY`: no existe release candidate desplegada en staging. Se documenta UAT técnico local de Leads; no se inicia UAT de producto ni se declara evidencia de staging.                                                                                                                                                                      | `docs/06-product/crm/shared/CRM_PILOT_READINESS_REVIEW.md`, documentación de Leads Fase 5                                                                                                                                                              |
| 2026-08-24 | Playwright y revisión visual         | Pendientes del gate final. La certificación UI exige aprobación visual explícita antes de Playwright; además, la infraestructura existente no incluye journeys de Leads ni proyecto tablet.                                                                                                                                                    | `playwright.config.mjs`, `e2e/`, `CRM_LEADS_UI_IMPLEMENTATION_PLAN.md`                                                                                                                                                                                 |

**Estado formal de Fase 5:** `in-progress`, implementación técnica validada
...
- **Fecha:** 2026-08-24.
- **Rama de continuación:** `feature/leads-frontend-implementation`.
- **Commit de partida:** `a3a8340` (`feat(crm): implement leads frontend phases one through four`), publicado en
  `origin/feature/leads-frontend-implementation`; los cambios de esta sesión quedan sin commit.
- **Estado alcanzado:** Leads UI Fases 1–4 con validación técnica ampliada; asignaciones restringidas a
  miembros operativos activos y carreras de captura por índice único reconciliadas como `reused`.
  Conversión cualificada idempotente permanece sobre el endpoint/RPC existente, sin cambios en Contacts.
- **Decisiones, bloqueos y riesgos:** `contactId` se hereda y no es editable; `qualified` y
  `lead_conversion` permanecen estables; la revisión visual y Playwright requieren aprobación
  explícita del usuario y bloquean la certificación UI/UX. UTM source y catálogo de asignados siguen
  diferidos por limitaciones del backend.
- **Validación ejecutada:** Leads focalizado `99/99`, suite serial `862/862`, typecheck, lint,
  shell `39/39`, frontend quality gate, registry, source-contracts, ownership, links, Supabase
  governance, track validator y `git diff --check`. `validate:changed` alcanza los controles hasta
  el build y se detiene por variables Supabase ausentes; `validate:ci` queda bloqueado por la misma
  causa. pgTAP está bloqueado por Docker/Postgres local; `docs:inventory:check` sigue desactualizado.
- **Siguiente acción concreta:** obtener aprobación visual explícita y ejecutar Playwright L1/L2/L3
  (sin marcar Fase 5 completa); después habilitar staging reproducible, aislamiento remoto y UAT.
