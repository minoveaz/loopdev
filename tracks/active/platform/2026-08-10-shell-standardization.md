...
Este programa se ejecuta mediante ramas de estandarización y composición. `feature/shell-standardization`
estableció los contratos y guardrails; `feature/suiteshell-composition`, publicada en `origin`, continúa
la composición declarativa con `SuiteRuntime` y `Shell Showcase`. Cada migración posterior debe declarar
su rama y evidencia en la fase correspondiente.
...
- Reclasificado como `active` tras revisar los commits de `feature/suiteshell-composition`, incluido
  `0bf5f51`, que implementa composición contextual, `SuiteRuntime` y el Showcase.
