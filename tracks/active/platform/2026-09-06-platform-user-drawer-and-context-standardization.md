---
id: platform-user-drawer-and-context-standardization
...
- [ ] Conectar `ContextPanelHost` con `useAuth()` y `useOrganization()`.
- [ ] Mostrar nombre, email, badge de rol y organización activa real.
- [ ] Conectar `Sign Out` con `signOut()` de Supabase y redirección a `/login`.
- [ ] Zona horaria automática mediante `Intl.DateTimeFormat().resolvedOptions().timeZone`.
- [ ] Botones de navegación interna (*Profile*, *Account Settings*, *Billing*) como placeholders no operativos declarados.

### Fase 2: Unificación de apertura de Drawer en Launchpad y Suites
- [ ] Conectar el avatar en `apps/loopdev-os/src/app/launchpad/page.tsx` para abrir el Drawer canónico de perfil.
- [ ] Proveer datos reales de sesión a `UserMenu` en `DocumentIntelligenceShell` y `SalesCrmShell`.

### Fase 3: Corrección de estado de carga en Launchpad ContextSwitcher
- [ ] Sincronizar el estado de carga con la resolución de sesión y membresías.
- [ ] Reemplazar el bloque rígido de "Loading context" con un estado de carga visual limpio y sin parpadeos.

## Criterios de cierre

- [ ] Clic en el avatar tanto en Launchpad como en Suites abre el Drawer lateral de perfil.
- [ ] El Drawer muestra la identidad real del usuario autenticado sin datos mock.
- [ ] El cierre de sesión desloguea la sesión real de Supabase y redirige a `/login`.
- [ ] El `ContextSwitcher` del Launchpad no se queda atascado en "Loading context".
- [ ] El icono de la bombilla en notificaciones permanece inalterado.
