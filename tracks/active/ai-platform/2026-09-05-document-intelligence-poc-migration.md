...
| 2026-09-06 | Diagnóstico y corrección de membership autenticada en navegador                                                             | ✅ causa raíz: `created_at` de `timestamptz` llega con offset `+00:00` y el contrato lo descartaba; la schema acepta offsets y el Workbench carga para 2/2 organizaciones                              | `packages/contracts/src/platform/tenancy.ts`; `tenancy.test.ts`; Playwright real                                       |
| 2026-09-06 | Smoke autenticado front/back con upload real y cookies SSR                                                                  | ✅ 2/2 organizaciones: login, membership, Workbench, multipart route y Edge Function responden sin error de auth; ambas respuestas del route fueron HTTP 200; cuentas y archivos temporales eliminados | Playwright real contra Supabase dev; `extract-identity-document` v15                                                   |
...
- **Decisiones, bloqueos y riesgos:** `GEMINI_API_KEY` permanece solo en Supabase. La revisión de
  ...
  paths privados, errores sanitizados y cleanup sin PII. La discrepancia de membership quedó
  resuelta al aceptar en `OrganizationMembershipSchema` los offsets UTC que devuelve Postgres;
  el Workbench autenticado carga ahora para ambas organizaciones. La matriz negativa RLS ya tiene
...
- **Siguiente acción concreta:** ejecutar el smoke front/back completo con cookies de navegador
  y ambas organizaciones, registrar su evidencia y cerrar la Fase 5 solo cuando extracción,
  cleanup y aislamiento queden confirmados.
