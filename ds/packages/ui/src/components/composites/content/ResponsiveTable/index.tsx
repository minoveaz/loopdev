...
import { Button, Checkbox, LpdText, Select, Spacer, TechnicalSurface } from '../../../atoms';
...
export function ResponsiveTable<Row extends object>({
  ...
  const isStateOnly =
    loading || Boolean(errorState) || offline || disabled || forbidden || rows.length === 0;
  ...
  const sortedRows = sortKey
    ? [...rows].sort((left, right) => {
        ...
        const leftValue =
          column?.sortAccessor?.(left) ?? (left as Record<string, unknown>)[sortKey];
        const rightValue =
          column?.sortAccessor?.(right) ?? (right as Record<string, unknown>)[sortKey];
  ...
  const allRowsSelected =
    selectableKeys.length > 0 && selectableKeys.every((key) => selectedRowKeys.includes(key));
    ...
    >
      {bulkActions && selectedRowKeys.length > 0 && (
        <div
          ...
          className="mb-3 flex min-h-12 items-center justify-between gap-3 border border-primary/30 bg-primary/5 px-3 py-2 shadow-sm"
        >
          <LpdText
            as="span"
            size="xs"
      ...
      >
        <table
          className={`${showAllColumnsOnMobile ? 'table' : 'hidden md:table'} w-full min-w-[980px] border-collapse text-left`}
        >
          ...
          <thead>
            <tr className="border-b-2 border-border-technical bg-shell-canvas shadow-[inset_0_-1px_0_var(--border-border-technical)]">
                ...
                >
                  {column.sortable ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1 hover:text-text-main disabled:cursor-not-allowed disabled:opacity-50"
              ...
              {rowActions ? (
                <th
                  scope="col"
                  aria-label={resolvedLabels.actions}
                  ...
                  return (
                    ...
                    >
                      ...
                      {columns.map((column) => (
                        <td
                          key={column.key}
                          className={`px-3 ${density === 'dense' ? 'py-2' : 'py-3'} ${column.className ?? ''}`}
                        >
                          {column.render
                            ? column.render(row)
                      ...
                      {rowActions ? (
                        <td
                          className={`w-10 px-3 ${density === 'dense' ? 'py-2' : 'py-3'} text-right`}
            ...
            {showMobileHeader && (
              <div className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-3 border-b-2 border-border-technical bg-shell-canvas px-3 py-2 text-xs text-text-muted">
                <span className="font-mono uppercase tracking-widest">
                  {resolvedLabels.mobileHeader.record}
                </span>
                <span className="font-mono uppercase tracking-widest">
                  {resolvedLabels.mobileHeader.status}
                </span>
                <span className="font-mono uppercase tracking-widest">
                  {resolvedLabels.mobileHeader.actions}
                </span>
              </div>
            )}
            {visibleRows.map((row, index) => (
      ...
      {!isStateOnly && pageSize > 0 ? (
        <>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t-2 border-border-technical bg-background-subtle px-3 py-3 text-xs text-text-muted">
            <span>
              {resolvedLabels.showing(
                sortedRows.length === 0 ? 0 : (activePage - 1) * pageSize + 1,
                Math.min(activePage * pageSize, sortedRows.length),
                sortedRows.length,
              )}
            </span>
            <div
              className={
                paginationVariant === 'compact'
                  ? 'grid w-full grid-cols-[auto_minmax(0,1fr)_minmax(0,1fr)_auto] items-center gap-1 sm:w-auto'
                  : 'flex flex-wrap items-center gap-1 rounded-sm border border-border-technical bg-background px-1 py-1'
              }
            >
              <button
                className={
                  paginationVariant === 'compact'
                    ? 'min-h-8 min-w-0 rounded-sm px-1 py-1 text-xs text-text-main hover:bg-background-subtle disabled:cursor-not-allowed disabled:opacity-50'
                    : 'min-h-9 rounded-sm border border-border-subtle bg-background px-3 py-1 text-text-main hover:bg-background-subtle disabled:cursor-not-allowed disabled:opacity-50'
                }
                type="button"
                disabled={activePage === 1 || !canInteract}
                onClick={() => setActivePage(Math.max(1, activePage - 1))}
              >
                {resolvedLabels.previous}
              </button>
              <Select
                aria-label={resolvedLabels.goToPage}
                size="sm"
                fullWidth={false}
                disabled={!canInteract}
                className={
              ...
              <button
                ...
                type="button"
                disabled={activePage === pageCount || !canInteract}
