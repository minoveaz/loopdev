...
export const ModuleContextSidebar: React.FC<ModuleContextSidebarProps> = ({
  ...
  return (
    <aside
      ...
      className={`${collapsed && collapsedPresentation === 'trigger' ? 'contents' : `border-border-technical bg-shell-canvas flex min-h-0 shrink-0 flex-col overflow-hidden border-r max-lg:max-h-64 max-lg:w-full max-lg:border-b max-lg:border-r-0 ${collapsed ? 'w-14' : widthClasses[width]}`} ${className}`}
    >
