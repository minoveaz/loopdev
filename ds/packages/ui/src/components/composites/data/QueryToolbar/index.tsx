...
export function QueryToolbar({
  ...
  const selectClass = 'h-8 min-w-28 rounded-md border border-border-technical bg-shell-surface px-2 text-xs text-text-main';
  ...
  return (
    ...
    >
      <FilterBar
        ...
        actions={actions}
      />
      {(resultCount !== undefined || sort || view) ? (
        <div className="flex min-w-0 flex-wrap items-center gap-2 text-xs text-text-muted">
          ...
          <div className="flex flex-wrap items-center gap-2 lg:ml-auto">
            ...
            {view ? (
              <div className="flex items-center gap-1" role="group" aria-label={view.label ?? 'View'}>
