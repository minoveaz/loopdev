...
export const useModuleWorkspace = (props: ModuleWorkspaceProps) => {
  ...
  const isOverlayMode = forceOverlay || manualIsMobile === true || isOverlayModeInternal;

  useEffect(() => {
    if (forceOverlay || manualIsMobile !== undefined) return;
  ...
  }, [breakpoint, forceOverlay, manualIsMobile]);
