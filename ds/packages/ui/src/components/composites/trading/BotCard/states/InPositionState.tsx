...
import type { BotStateActions } from './types';

interface InPositionStateProps extends BotStateActions {
  bot: any;
}
    ...
    >
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <LpdText
              size="xs"
              weight="black"
              className="uppercase tracking-widest text-text-muted opacity-60 leading-none"
            >
              Status: IN_POSITION
            </LpdText>
      ...
      <div className="flex justify-between items-center px-1 border-b border-white/5 pb-4">
        <div className="flex gap-8">
          <div className="flex flex-col gap-1">
            <LpdText size="nano" weight="black" className="uppercase text-text-muted opacity-40">
              Capital_Deployed
            </LpdText>
            <LpdText size="sm" weight="black" className="font-mono text-text-main">
              ${Number(bot.baseInvestmentUsdt || 0).toLocaleString()}
            </LpdText>
          </div>
          <div className="flex flex-col gap-1">
            <LpdText size="nano" weight="black" className="uppercase text-text-muted opacity-40">
            ...
            <LpdText size="sm" weight="black" className="font-mono text-text-main/80">
              -${(Number(bot.baseInvestmentUsdt || 0) * 0.002).toFixed(2)}{' '}
              <span className="text-[9px] opacity-40">(0.20%)</span>
            </LpdText>
          </div>
        </div>
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <div className="flex items-center gap-2 bg-primary/5 px-3 py-1 rounded-full border border-primary/10">
            ...
            <LpdText
              size="xs"
              weight="black"
              className="font-mono text-primary animate-pulse tracking-tighter"
            >
              {elapsedTime}
            </LpdText>
          </div>
      ...
      <div className="grid grid-cols-2 gap-x-10 gap-y-8 pt-6 border-t border-white/5">
        <div className="flex flex-col gap-2">
          ...
          <LpdText size="base" weight="black" className="font-mono text-text-main opacity-90">
            ${formatPrice(bot.currentEntryPrice)}
          </LpdText>
          <LpdText size="nano" className="text-text-muted font-mono opacity-30 italic uppercase">
            Tick_Verified
          </LpdText>
        </div>
        ...
        <div className="flex flex-col gap-2 text-right">
          ...
          <LpdText size="base" weight="black" className="font-mono text-primary opacity-90">
            ${formatPrice(bePrice)}
          </LpdText>
          <LpdText size="xs" weight="black" className="text-primary opacity-80 font-mono">
            {getRelPct(bePrice)}% / {getRelUsd(bePrice)}
          </LpdText>
        </div>
          ...
          <div className="flex items-center gap-2">
            <LpdText
              ...
              className={cn(
                ...
                slPrice > bot.currentEntryPrice ? 'text-emerald-500/50' : 'text-rose-500/50',
              )}
            >
              Stop_Loss
            </LpdText>
            <LpdText
              ...
              className={cn(
                'font-mono opacity-80',
                slPrice > bot.currentEntryPrice ? 'text-emerald-500' : 'text-rose-500',
              )}
            >
        ...
        <div className="flex flex-col gap-2 text-right">
          <LpdText
            size="nano"
            weight="black"
            className="uppercase text-emerald-500/50 tracking-widest"
          >
            Target_TP
          </LpdText>
          <LpdText size="base" weight="black" className="font-mono text-emerald-500 opacity-90">
            ${formatPrice(tpPrice)}
          </LpdText>
          <LpdText size="xs" weight="black" className="text-emerald-500 opacity-80 font-mono">
            {getRelPct(tpPrice)}% / {getRelUsd(tpPrice)}
          </LpdText>
        </div>
      </div>
    </div>
