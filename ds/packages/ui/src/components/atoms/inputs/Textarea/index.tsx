...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(({
  ...
  return (
    <div className={cn('flex flex-col gap-1.5', fullWidth ? 'w-full' : 'w-fit', className)}>
      {label && <label htmlFor={textareaId} className="text-sm font-black uppercase tracking-widest text-text-muted">{label}</label>}
      <textarea
        {...props}
        ref={ref}
        id={textareaId}
      ...
      />
      {(error || helperText) && <span id={descriptionId} className={cn('text-xs', error ? 'text-danger' : 'text-text-muted')}>{error ?? helperText}</span>}
    </div>
  );
});
