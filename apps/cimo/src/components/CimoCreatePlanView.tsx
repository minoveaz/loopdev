...
export const CimoCreatePlanView: React.FC<CimoCreatePlanViewProps> = ({
  ...
  // Auto-generate comprehensive smart description synthesizing all user choices
  const autoGeneratedDescription = useMemo(() => {
    const sportName = selectedSportObj.label;
    const loc = cleanLocationName || location.split('(')[0].trim() || 'punto de encuentro';
    const paceStr = `${currentPace.title} (${currentPace.metric})`;

    // 1. Sport, Location, City & Group Size
    let text = `Convocatoria de ${sportName} en ${loc} (${selectedCity}) para un grupo de hasta ${maxMembers} personas. Nivel ${paceStr}.`;

    // 2. Schedule
    text += ` Nos reuniremos el ${date} a las ${time}h.`;

    // 3. Captain Instructions if provided
    if (instructions.trim()) {
      text += ` 📋 Recomendación del Capitán: ${instructions.trim().replace(/\.$/, '')}.`;
    }
    ...
    if (gearItems.length > 0) {
      ...
      text += ` 🎒 Material imprescindible: ${gearList}.`;
    }
    ...
    if (hasThirdHalf) {
      ...
      const venueName = thirdHalfVenue.split('(')[0].trim() || 'terraza cercana';
      text += ` ☕ Dimensión Social: Tras el entreno haremos ${thLabel} en ${venueName}`;
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="bg-white border-2 border-[#7FB77E]/40 rounded-3xl p-6 sm:p-8 shadow-sm flex flex-col gap-6 relative overflow-hidden">
          ...
          <div className="flex flex-col gap-2.5 pt-4 border-t border-[#1F4E5F]/10">
            ...
            <p className="text-xs text-[#1F4E5F]/60 font-medium">
              Hemos recopilado toda la información de tu deporte, horario, punto de encuentro, normas y tercer tiempo. Puedes aceptarla o editar cualquier frase directamente:
            </p>
            ...
            <textarea
              ...
              className="w-full p-4 rounded-2xl border border-[#1F4E5F]/20 focus:border-[#7FB77E] focus:ring-2 focus:ring-[#7FB77E]/20 text-xs sm:text-sm font-medium text-[#1F4E5F] outline-none bg-white shadow-2xs resize-none leading-relaxed transition-all"
            />
          </div>
