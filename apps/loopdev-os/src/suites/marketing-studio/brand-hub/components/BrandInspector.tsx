...
export const BrandInspector: React.FC<BrandInspectorProps> = ({ tab, context, isLoading }) => {
  ...
  if (entity.type === 'color.token') {
    const token = activeBrand?.palette?.tokens.find((t) => t.id === entity.id);
    if (token) {
      ...
      <div className="px-6">
        ...
        <div className="py-4">
          ...
          <LpdText size="sm" className="text-text-muted leading-relaxed">
            This entity defines the core identity attributes for {entity.name}. Changes here
            propagate to all consumer modules.
          </LpdText>
        </div>
      </div>
    );
