...
| PDF file | Browser-local PDF | PDF.js legacy build with a bundler-resolved worker; the first render derives `baseFitScale`, then a conservative non-white bounding-box pass derives `autoFitScale` for pages with excessive surrounding whitespace. Normal and blank pages retain page fit; iframe fallback fills the viewport on render failure | Same view controls except crop; `userZoom` starts at `1.0`/`100%`, reset returns to that base, and zoom/pan remain relative to the visual result | Canvas/iframe has the file name and preserves content aspect ratio |
