...
function relationHref(task: Task) {
  return task.relationType === 'contact'
    ? `/sales-crm/contacts/${task.relationId}`
    : task.relationType === 'lead'
      ? `/sales-crm/leads/${task.relationId}`
      : `/sales-crm/opportunities/${task.relationId}`;
}
...
export function TaskForm() {
  ...
  const [relationType, setRelationType] = useState<TaskRelationType>(
    (searchParams.get('relationType') as TaskRelationType) || 'contact',
  );
          ...
          >
            ...
            <div className="flex justify-end gap-2">
              <Button
                type="button"
                variant="secondary"
                onClick={() => router.push('/sales-crm/tasks')}
              >
                Cancel
              </Button>
              <Button
                type="submit"
                variant="primary"
                disabled={isSaving || !title || !relationId || !dueAt}
              >
  ...
  useEffect(() => {
    ...
    fetch(`/api/crm/tasks?organizationId=${encodeURIComponent(activeOrganizationId)}&limit=100`, {
      ...
      .catch((requestError: unknown) => {
        if (requestError instanceof DOMException && requestError.name === 'AbortError') return;
        setError(
          requestError instanceof Error ? requestError.message : 'My Day could not be loaded.',
        );
      })
      .finally(() => {
  ...
  const groups = useMemo(() => {
    ...
    return {
      overdue: tasks.filter(
        (task) =>
          task.status !== 'completed' && task.dueAt && new Date(task.dueAt).getTime() < start,
      ),
      today: tasks.filter(
        (task) =>
          task.status !== 'completed' &&
          task.dueAt &&
          new Date(task.dueAt).getTime() >= start &&
          new Date(task.dueAt).getTime() < end,
      ),
      upcoming: tasks.filter(
        (task) =>
          task.status !== 'completed' && task.dueAt && new Date(task.dueAt).getTime() >= end,
      ),
      ...
      <main className="min-h-0 flex-1 overflow-auto p-4 lg:p-6">
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {(['overdue', 'today', 'upcoming', 'withoutDueDate'] as const).map((key) => (
              <TechnicalSurface
                key={key}
                variant="surface"
                radius="md"
                border={key === 'overdue' ? 'technical' : 'subtle'}
                className="p-4"
              >
                <div className="flex items-center justify-between">
                  ...
                  <Badge
                    status={key === 'overdue' ? 'error' : 'neutral'}
                    variant="outline"
                    showDot={false}
                  >
                    {groups[key].length}
                  ...
                  <ul className="mt-3 space-y-2">
                    {groups[key].map((task) => (
                      ...
                      >
                        <Link
                          href={`/sales-crm/tasks/${task.id}`}
                          className="text-text-main text-sm font-medium underline-offset-2 hover:underline"
                        >
                          {task.title}
                        </Link>
                        <p className="text-text-muted mt-1 text-xs">
                          {task.priority} · {formatDate(task.dueAt)}
                        </p>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="text-text-muted mt-3 text-sm">Nothing here.</p>
                )}
              </TechnicalSurface>
            ))}
          </div>
