...
export async function createConversation(input: CreateCommunicationConversationCommand) {
  ...
  if (
    ...
    !contact.data ||
    !channel.data ||
    channel.data.contact_id !== parsed.contactId
  ) {
  ...
  if (parsed.brandId) {
    ...
    if (brand.error || !brand.data)
      throw new Error('Communication brand does not belong to the organization');
  }
  if (parsed.workspaceId) {
    ...
    if (workspace.error || !workspace.data)
      throw new Error('Communication workspace does not belong to the organization');
  }
...
export async function sendWhatsAppConversationText(input: {
  ...
  const conversation = await supabase
    .from('communication_conversations')
    .select('id, channel_id, window_expires_at')
  ...
  if (
    ...
    Date.parse(conversation.data.window_expires_at) <= Date.now()
  ) {
...
export async function sendWhatsAppConversationTemplate(input: {
  ...
  const parameters = resolveWhatsAppTemplateParameters(parameterNames, input.templateParameters);
...
export async function createInternalNote(
  input: CreateCommunicationInternalNoteCommand & { authorId: string },
) {
  ...
  const { data, error } = await supabase
    .from('communication_internal_notes')
    .insert({
      organization_id: parsed.organizationId,
      conversation_id: parsed.conversationId,
      author_id: authorId,
...
export async function registerWebhookEvent(
  ...
  if (error) {
    ...
    if (existing.error || !existing.data)
      throw new Error('Unable to register communication webhook event');
...
export async function scheduleMessageRetry(input: RetryCommunicationMessageCommand) {
  ...
  if (message.status !== 'failed')
    throw new Error('Only failed communication messages can be retried');
  if (message.retry_count >= message.max_retries)
    throw new Error('Communication message retry limit reached');
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      last_error_code: parsed.errorCode ?? null,
    })
    .eq('organization_id', parsed.organizationId)
    .eq('id', parsed.messageId)
    .eq('status', 'failed')
