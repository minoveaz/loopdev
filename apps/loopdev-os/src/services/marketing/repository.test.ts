...
import {
  marketingFixtureAssets,
  marketingFixtureBrandSnapshots,
  marketingFixtureConnections,
  marketingFixtureIds,
} from './fixtures/marketing-data';
...
describe('InMemoryMarketingRepository', () => {
  ...
  it('rejects reads and writes without the corresponding grant', async () => {
    ...
    await expect(repository.createCampaign(context, input)).rejects.toBeInstanceOf(
  ...
  it('creates a campaign only inside the context scope', async () => {
    ...
    await expect(
      repository.createCampaign(context, { ...input, workspaceId: ids.otherWorkspaceId }),
  ...
  it('updates an authorized campaign and rejects cross-scope mutations', async () => {
    ...
    await expect(
      repository.updateCampaign(context, {
        organizationId: ids.organizationId,
  ...
  it('reads Brand Hub, assets and connections only for the authorized brand', async () => {
    ...
    expect((await repository.getBrandSnapshot(context))?.brand.id).toBe(
      marketingFixtureIds.vitablueBrand,
    );
    expect(await repository.listAssets(context)).toHaveLength(1);
    expect(await repository.listConnections(context)).toHaveLength(1);
    expect((await repository.listConnections(context))[0]?.organizationId).toBe(
      marketingFixtureIds.vitablueOrganization,
    );
  });
  ...
  it('revokes a connection only with manage permission and matching scope', async () => {
    ...
    const revoked = await repository.disconnectConnection(
      context,
      marketingFixtureIds.vitablueConnection,
    );
    expect(revoked.status).toBe('revoked');
    expect(
      await repository.listConnections({
        ...context,
        grants: [{ ...context.grants[0], permission: 'read' as const }],
      }),
    ).toHaveLength(1);
    await expect(
      repository.disconnectConnection(
        { ...context, brandId: ids.otherBrandId },
        marketingFixtureIds.vitablueConnection,
      ),
    ).rejects.toBeInstanceOf(MarketingAccessDeniedError);
  });
  ...
  it('approves a draft campaign only with explicit approve permission', async () => {
    ...
    await expect(
      repository.approveCampaign(
        { ...context, grants: [{ ...context.grants[0], permission: 'edit' as const }] },
        campaignId,
      ),
    ).rejects.toBeInstanceOf(MarketingAccessDeniedError);
  });
  ...
  it('manages asset metadata and variants inside the authorized scope', async () => {
    ...
    expect(
      (await repository.updateAssetMetadata(context, asset.id, { name: 'Updated hero' })).name,
    ).toBe('Updated hero');
    ...
    await expect(
      repository.updateAssetMetadata({ ...context, grants: [grant] }, asset.id, { name: 'Denied' }),
    ).rejects.toBeInstanceOf(MarketingAccessDeniedError);
    await expect(
