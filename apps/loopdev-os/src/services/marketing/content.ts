import { ContentBriefSchema, ContentItemSchema, ContentVersionSchema, CreateContentBriefSchema, CreateContentItemSchema, CreateContentVersionSchema } from '@loopdev/contracts';
import type { ContentBrief, ContentItem, ContentVersion, CreateContentBriefInput, CreateContentItemInput, CreateContentVersionInput } from '@loopdev/contracts';
...
export async function listContentItems(organizationId: string, brandId: string, workspaceId?: string) {
  const supabase = await createServerSupabaseClient();
  let query = supabase.from('content_items').select('*').eq('organization_id', organizationId).eq('brand_id', brandId).order('updated_at', { ascending: false });
  if (workspaceId) query = query.eq('workspace_id', workspaceId);
  const { data, error } = await query;
...
export async function createContentItem(input: CreateContentItemInput, userId: string) {
  ...
  if (!brandVersionId && parsed.briefId) {
    const { data: brief } = await supabase.from('content_briefs').select('brand_version_id').eq('id', parsed.briefId).eq('organization_id', parsed.organizationId).maybeSingle();
    brandVersionId = brief?.brand_version_id ?? null;
  }
  const { data, error } = await supabase.from('content_items').insert({
    organization_id: parsed.organizationId, brand_id: parsed.brandId, brand_version_id: brandVersionId,
    workspace_id: parsed.workspaceId ?? null, campaign_id: parsed.campaignId ?? null, brief_id: parsed.briefId ?? null,
    type: parsed.type, title: parsed.title, locale: parsed.locale, status: parsed.status,
    current_version: parsed.currentVersion, created_by: userId, updated_by: userId,
  }).select('*').single();
...
export async function createContentVersion(input: CreateContentVersionInput, userId: string) {
  ...
  const { data, error } = await supabase.from('content_versions').insert({
    organization_id: parsed.organizationId, brand_id: parsed.brandId, workspace_id: parsed.workspaceId ?? null,
    content_item_id: parsed.contentItemId, version: parsed.version, body: parsed.body,
    change_summary: parsed.changeSummary ?? null, created_by: userId,
  }).select('*').single();
...
function mapItem(row: Record<string, unknown>): ContentItem {
  return ContentItemSchema.parse({ id: row.id, organizationId: row.organization_id, brandId: row.brand_id, brandVersionId: row.brand_version_id, workspaceId: row.workspace_id, campaignId: row.campaign_id, briefId: row.brief_id, type: row.type, title: row.title, locale: row.locale, status: row.status, currentVersion: row.current_version, createdBy: row.created_by, updatedBy: row.updated_by, createdAt: row.created_at, updatedAt: row.updated_at });
}

function mapVersion(row: Record<string, unknown>): ContentVersion {
  return ContentVersionSchema.parse({ id: row.id, organizationId: row.organization_id, brandId: row.brand_id, workspaceId: row.workspace_id, contentItemId: row.content_item_id, version: row.version, body: row.body, changeSummary: row.change_summary, createdBy: row.created_by, updatedBy: null, createdAt: row.created_at, updatedAt: row.created_at });
}
