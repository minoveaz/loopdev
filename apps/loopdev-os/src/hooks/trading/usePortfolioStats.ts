...
export const usePortfolioStats = () => {
  ...
  const activeBotsCount = bots.filter(
    (b) => b.status === 'active' || b.status === 'paper_trading',
  ).length;
  ...
  const unrealizedPnlPct = deployedCapital > 0 ? (unrealizedPnlUsdt / deployedCapital) * 100 : 0;
