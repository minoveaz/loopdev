...
interface BalanceResponse {
  success: boolean;
  available_trading_usdt: number;
}

async function readResponse<T>(response: Response): Promise<T> {
  const body = await response.json().catch(() => ({}));
  if (!response.ok)
    throw new Error(typeof body.error === 'string' ? body.error : 'Exchange vault request failed');
  return body as T;
}
...
export const useExchangeVault = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/quant/exchanges?organizationId=${encodeURIComponent(activeOrganization!.id)}`,
      );
      ...
      return exchanges.map((exchange): ExchangeAccount => ({
        ...
        status: exchange.last_error_message
          ? 'error'
          : exchange.is_active && exchange.last_verified_at
            ? 'healthy'
            : exchange.is_active
              ? 'unknown'
              : 'disconnected',
        isPaper: true,
        lastSync: exchange.last_verified_at
          ? new Date(exchange.last_verified_at).toLocaleString()
          : 'Never verified',
  ...
  const connectExchange = useMutation({
    mutationFn: async (payload: ConnectPayload) => {
      if (!activeOrganization)
        throw new Error('Select an organization before connecting an exchange');
      return readResponse<ExchangeResponse>(
        await fetch('/api/quant/exchanges', {
          method: 'POST',
  ...
  const testConnection = useMutation({
    mutationFn: async (id: string) =>
      readResponse<TestConnectionResponse>(
        await fetch(`/api/quant/exchanges/${id}/test`, { method: 'POST' }),
      ),
    onSuccess: () => queryClient.invalidateQueries({ queryKey }),
  });
  ...
  return {
    ...
    testConnection: (id: string, callbacks?: TestConnectionCallbacks) =>
      testConnection.mutate(id, callbacks),
