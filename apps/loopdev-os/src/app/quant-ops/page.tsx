...
import type { PositionData } from '@loopdev/ui';
...
export default function QuantOpsOverview() {
  ...
  const activePositionData: PositionData[] = activePositions.map((bot) => ({
    id: bot.id,
    pair: bot.pair,
    side: (bot.currentPositionSide ?? 'LONG') as 'LONG' | 'SHORT',
    strategy: bot.strategyName ?? 'Unknown Strategy',
    entryPrice: bot.currentEntryPrice,
    currentPrice: bot.currentPrice,
    quantity: bot.currentQuantity ?? 0,
    valueUsdt: bot.baseInvestmentUsdt,
    ...
    <main className="h-full overflow-y-auto flex flex-col gap-8 p-8 max-w-[1600px] mx-auto animate-in fade-in duration-700 pb-32 custom-scrollbar">
      ...
      <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <MetricCard
          ...
          delta={`${unrealizedPnlPct >= 0 ? '+' : ''}${unrealizedPnlPct.toFixed(2)}%`}
          trend={unrealizedPnlPct >= 0 ? 'up' : 'down'}
          icon="account_balance_wallet"
        />
        <MetricCard
          label="Aggregated PnL"
          value={`${unrealizedPnlUsdt >= 0 ? '+' : ''}$${unrealizedPnlUsdt.toLocaleString()}`}
          trend={unrealizedPnlUsdt >= 0 ? 'up' : 'down'}
          colorClassName={unrealizedPnlUsdt >= 0 ? 'text-emerald-500' : 'text-rose-500'}
          icon="trending_up"
        />
        <MetricCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 flex flex-col gap-8">
          ...
          <RiskMeter
            ...
            maxValue={10000}
            valueLabel={`$${deployedCapital.toLocaleString()}`}
            maxLabel="/ $10,000"
          />
        </div>
      </div>
          ...
          {bots.map((bot) => (
            <BotCardIndustrial
              ...
              onMarketExit={async (id) => {
                toast.show({
                  tenantId: 'loopdev',
                  title: 'MARKET_EXIT_ISSUED',
                  variant: 'warning',
                  metadata: 'EXEC_MANUAL',
                });
              ...
              onSetToBE={async (id) => {
                ...
                toast.show({
                  ...
                  description: 'Moved to Break-even',
                  variant: 'success',
                  metadata: 'RISK_0',
                });
              }}
      ...
      <section className="flex flex-col gap-4 mt-8">
        ...
        <PositionsDataTable data={activePositionData} onViewDetail={(id) => openBotInspector(id)} />
      </section>
      ...
      <TechnicalDialog
        ...
        actions={
          <>
            <Button variant="ghost" onClick={() => setIsKillSwitchConfirmOpen(false)}>
              Abort
            </Button>
            <Button
              variant="danger"
