...
export const SessionPulse: React.FC<SessionPulseProps> = ({ bot }) => {
  ...
  const sessionPnlPct =
    bot.baseInvestmentUsdt > 0 ? (sessionPnlUsdt / bot.baseInvestmentUsdt) * 100 : 0;
  ...
  const winRate =
    bot.totalTrades > 0 ? ((bot.winningTrades / bot.totalTrades) * 100).toFixed(1) : '---';
    ...
    <section className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <IndustrialMetric
          ...
          secondaryValue={
            bot.totalTrades > 0
              ? `${bot.winningTrades}W / ${bot.totalTrades - bot.winningTrades}L`
              : 'IDLE_SESSION'
          }
        />
        <IndustrialMetric
