...
export const DeployBotModal: React.FC<DeployBotModalProps> = ({
  ...
  const isBalanceInsufficient =
    availableBalance !== null && formData.baseInvestmentUsdt > availableBalance;
  const isFormValid =
    formData.name &&
    formData.exchangeId &&
    formData.pair &&
    formData.strategyId &&
    formData.baseInvestmentUsdt > 0;
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 md:p-8">
      ...
      <TechnicalSurface
        variant="surface"
        depth="overlay"
        className="animate-in zoom-in-95 relative z-10 flex h-full max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden shadow-2xl duration-300"
      >
          ...
          >
            ...
            <div className="flex flex-col gap-6">
              <div className="flex items-center justify-between px-1">
                <LpdText
                  size="nano"
                  weight="black"
                  className="text-primary uppercase tracking-[0.2em] opacity-60"
                >
                  02. Strategy_Blueprint
                </LpdText>

                {/* TRADING STYLE BADGE */}
                {formData.strategyId &&
                  strategies.find((s) => s.id === formData.strategyId)?.tradingStyle && (
                    <Badge
                      variant="outline"
                      className={cn(
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between px-1">
                    ...
                    {availableBalance !== null && (
                      <LpdText
                        ...
                        className={cn(
                          'font-mono italic',
                          formData.baseInvestmentUsdt > availableBalance
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, baseInvestmentUsdt: Number(e.target.value) })
                    }
                    required
                  />
                </div>
            ...
            <div className="flex flex-col gap-6">
              <LpdText
                size="nano"
                weight="black"
                className="px-1 uppercase tracking-[0.2em] text-status-error opacity-60"
              >
                03. Risk_Guard_Parameters
              </LpdText>
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div className="flex flex-col gap-2">
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, maxDailyLossPct: Number(e.target.value) })
                    }
                  />
                  <LpdText size="nano" className="text-text-muted px-1 font-mono italic opacity-40">
                    Est. Risk: -$
                    {((formData.baseInvestmentUsdt * formData.maxDailyLossPct) / 100).toFixed(
                      2,
                    )}{' '}
                    USDT
                  </LpdText>
                ...
                <div className="flex flex-col gap-2">
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, globalStopLossPct: Number(e.target.value) })
                    }
                  />
                  <LpdText size="nano" className="text-text-muted px-1 font-mono italic opacity-40">
                    Est. Protection: -$
                    {((formData.baseInvestmentUsdt * formData.globalStopLossPct) / 100).toFixed(
                      2,
                    )}{' '}
                    USDT
                  </LpdText>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, maxExposureUsdt: Number(e.target.value) })
                  }
                />
              </div>
            </div>
