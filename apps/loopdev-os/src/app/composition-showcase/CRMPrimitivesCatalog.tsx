...
const COMMAND_DIALOG_GROUPS = [
  ...
  },
  {
    ...
    commands: [
      ...
      {
        id: 'connect',
        label: 'Connect to your project',
        shortcut: 'O then C',
        icon: <Icon name="cable" size="sm" />,
        disabled: true,
      },
    ],
  },
...
const INPUT_STATES = [
  ...
  { id: 'focused', label: 'Focused', props: { defaultValue: 'Northstar Labs', autoFocus: true } },
  {
    id: 'error',
    label: 'Error',
    props: { defaultValue: 'Unknown contact', error: 'Contact could not be found' },
  },
  { id: 'disabled', label: 'Disabled', props: { defaultValue: 'Read-only value', disabled: true } },
  {
    id: 'loading',
    label: 'Loading',
    props: { defaultValue: 'Searching contacts', isLoading: true },
  },
...
const BADGE_CASES = [
  ...
  { id: 'success', label: 'Active', props: { status: 'success' as const, showDot: true } },
  {
    id: 'error',
    label: 'Blocked',
    props: { status: 'error' as const, variant: 'outline' as const },
  },
  {
    id: 'live',
    label: 'Processing',
    props: { status: 'energy' as const, isLive: true, isTechnical: true },
  },
] satisfies Array<{
  id: string;
  label: string;
...
const EMPTY_STATE_CASES = [
  {
    ...
    description: 'Create a contact to begin building the CRM list.',
  ...
  },
  {
    ...
    description: 'Clear the active filters to recover the full list.',
    status: 'default' as const,
  },
  {
    id: 'error',
    ...
    description: 'The contact list could not be loaded.',
...
const AVATAR_CASES = [
  { id: 'initials', label: 'Initials', props: { name: 'Alex Morgan', size: 'md' as const } },
  {
    id: 'status',
    label: 'With status',
    props: { name: 'Maya Chen', size: 'md' as const, status: 'online' as const, withStatus: true },
  },
...
const CATALOG: CatalogItem[] = [
        ...
        <div className="mt-5 space-y-2 border-t border-border-subtle pt-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Input
              ...
              endIcon={<Icon name="calendar_today" size="sm" />}
        ...
        <div className="space-y-2 border-t border-border-subtle pt-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="primary" startIcon="add">
              <span className="font-mono">Start icon {'{startIcon: add}'}</span>
            </Button>
            <Button endIcon="arrow_forward" variant="outline">
              <span className="font-mono">End icon {'{endIcon: arrow_forward}'}</span>
            </Button>
            <Button variant="primary" isLoading>
              <span className="font-mono">Loading {'{isLoading: true}'}</span>
            </Button>
            <Button variant="secondary" disabled>
              <span className="font-mono">Disabled {'{disabled: true}'}</span>
            </Button>
            <Button fullWidth variant="secondary">
              <span className="font-mono">
                Full width {'{fullWidth: true, variant: secondary}'}
              </span>
            </Button>
          </div>
        </div>
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-2 border-t border-border-subtle pt-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <IconButton
              icon="more_vert"
              tooltip="Loading {isLoading: true}"
              ariaLabel="Loading {isLoading: true}"
              isLoading
            />
            <IconButton
              icon="more_vert"
              tooltip="Disabled {disabled: true}"
              ariaLabel="Disabled {disabled: true}"
              disabled
            />
          </div>
        </div>
  ...
  },
  {
    ...
    name: 'FilterDropdown',
    priority: 'P0',
    usage: 'Single- and multi-select filters',
    render: ({ selectedFilter, setSelectedFilter }) => (
      <div data-testid="crm-filter-dropdown-fixture">
        <FilterDropdown
          icon="filter_list"
          label="Segment"
          options={['Enterprise', 'SMB', 'Partner']}
  ...
  },
  {
    ...
    name: 'Select',
    priority: 'P0',
    usage: 'Native select when a CRM contract requires it',
    render: () => (
      <div className="grid gap-3 sm:grid-cols-2">
        {SELECT_CASES.map(({ id, label, props }) => (
          <Select key={id} {...props} aria-label={`Contact status ${label}`}>
  ...
  },
  {
    ...
    name: 'Checkbox',
    priority: 'P0',
    usage: 'Custom selection control for table compositions',
    render: ({ contactSelected, setContactSelected }) => (
      <div className="space-y-2">
        <Checkbox
  ...
  {
    id: 'badge',
    name: 'Badge / TechnicalStatusBadge',
    priority: 'P0',
    usage: 'Contact, pipeline and task status',
    render: () => (
      <div className="flex flex-wrap items-center gap-2">
        {BADGE_CASES.map(({ id, label, props }) => (
          <Badge key={id} {...props}>
            {label}
          </Badge>
        ))}
        <TechnicalStatusBadge label="ACTIVE" severity="success" withPulse />
      </div>
    ),
  },
  {
    ...
    name: 'EmptyState',
    priority: 'P0',
    usage: 'Empty and filtered-empty recovery',
    render: () => (
  ...
  },
  {
    ...
    priority: 'P0',
    usage: 'Loading without layout shift',
    render: () => (
  ...
  },
  {
    ...
    name: 'PageHeader',
    priority: 'P1',
    usage: 'CRM view heading and primary action',
    render: () => (
      <div className="space-y-5">
            ...
            <PageHeader
              ...
              description={
                index === 0
                  ...
                  : 'Review ownership, activity and lifecycle information across the customer relationship workspace.'
              }
              actions={
  ...
  },
  {
    ...
    name: 'UserAvatar',
    priority: 'P1',
    usage: 'Owner and assignee identity',
    render: () => (
  ...
  {
    id: 'command-bar',
    name: 'CommandBarTrigger',
    priority: 'P1',
    usage: 'Only when the CRM shell consumes it',
  ...
  },
  {
    ...
    name: 'CommandDialog',
    priority: 'P1',
    usage: 'Command search overlay owned by the CRM shell',
    render: ({ commandDialogOpen, setCommandDialogOpen }) => (
      <div data-testid="crm-command-dialog-fixture" className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <CommandBarTrigger
            placeholder="Search or type a command..."
            onOpen={() => setCommandDialogOpen(true)}
        ...
        <CommandDialog
          ...
          commands={[]}
          groups={COMMAND_DIALOG_GROUPS}
          title="Command palette"
          description="Available CRM workspace commands"
  ...
  },
  {
    ...
    priority: 'P2',
    usage: 'Deferred until a CRM composition requires it',
    render: () => (
      <TrailingControl currentDistance={0.5} onUpdateDistance={async () => undefined} />
    ),
  },
];
...
export function CRMPrimitivesCatalog() {
  ...
  const groups = (['P0', 'P1', 'P2'] as Priority[]).map((priority) => ({
    priority,
    items: CATALOG.filter((item) => item.priority === priority),
  }));

  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="CRM primitive inventory"
        title="Shared components"
        description="Current rendered contracts before CRM composition certification."
      />
      {groups.map(({ priority, items }) => (
        <section key={priority} aria-labelledby={`crm-${priority}`} className="space-y-3">
          <SectionHeader
            title={`${priority} · ${priority === 'P0' ? 'Required baseline' : priority === 'P1' ? 'Composition support' : 'Deferred candidate'}`}
          />
