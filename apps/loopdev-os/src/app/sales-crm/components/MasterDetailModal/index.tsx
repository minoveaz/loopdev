...
const WHATSAPP_TEMPLATES = [
  {
    ...
    text: (name: string) => `Hola ${name}, gracias por tu interés en LoopDev Salud. Soy Elena, tu asesora comercial. ¿Tienes unos minutos hoy para conversar sobre nuestros planes?`
  },
  {
    id: 'proposal',
    label: 'Envío de Propuesta',
    text: (name: string) => `Hola ${name}, acabo de enviarte la cotización formal a tu correo. Quedo atenta para resolver cualquier duda que tengas sobre la propuesta comercial.`
  },
  {
    id: 'followup',
    label: 'Seguimiento de Trato',
    text: (name: string) => `Hola ${name}, te escribo para saber si pudiste revisar la cotización que te enviamos y si hay algún ajuste que debamos realizar.`
  },
  {
    id: 'meeting',
    label: 'Recordatorio de Cita',
    text: (name: string) => `Hola ${name}, te recuerdo que tenemos agendada nuestra sesión técnica hoy en 30 minutos. Te comparto el enlace de la reunión en un momento.`
  }
];
...
function MasterDetailModalContent({ onClose }: { onClose: () => void }) {
  ...
  const getWhatsAppLatency = () => {
    if (!lastWhatsAppLog) {
      return { text: 'Sin chats previos registrados', colorClass: 'text-slate-400 dark:text-slate-500' };
    }
    const diffTime = Math.abs(new Date().getTime() - new Date(lastWhatsAppLog.timestamp).getTime());
    const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
    
    if (diffDays < 1) {
      return { text: 'Último contacto: Hoy', colorClass: 'text-emerald-600 dark:text-emerald-400 font-bold' };
    } else if (diffDays === 1) {
      return { text: 'Último contacto: Ayer', colorClass: 'text-slate-500 dark:text-slate-400 font-semibold' };
    } else if (diffDays <= 3) {
      return { text: `Último contacto: hace ${diffDays} días`, colorClass: 'text-slate-500 dark:text-slate-400' };
    } else {
      return { text: `Alerta: último contacto hace ${diffDays} días`, colorClass: 'text-rose-500 dark:text-rose-400 font-bold animate-pulse' };
    }
  };
  ...
  const renderFooterActions = () => {
    const discardBtn = (
      ...
      >
        <span className="flex items-center gap-1.5"><Icon name={ICON_REGISTRY.actions.close} size="sm" /> Descartar</span>
      </Button>
    );
    ...
    >
      ...
      {showClosureForm && (
        <div className="absolute inset-0 bg-slate-950/40 dark:bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <TechnicalSurface variant="surface" className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl max-w-md w-full flex flex-col gap-4 text-xs font-sans">
            <div className="flex flex-col gap-1 border-b border-slate-200 dark:border-white/5 pb-2">
              ...
              <p className="text-[10px] text-slate-500 dark:text-slate-400">Por favor, registra el motivo de la baja del trato.</p>
            </div>
            <form onSubmit={handleConfirmClosure} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                <label className="text-slate-500 dark:text-slate-400 font-bold font-mono text-[10px]">MOTIVO DE CIERRE</label>
                <select 
                  value={closureReason} 
                ...
                >
                  {showClosureForm === 'rejected' ? (
                    <>
                      ...
                      <option value="Contratado con otra compañía">Contratado con otra compañía</option>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-slate-500 dark:text-slate-400 font-bold font-mono text-[10px]">DETALLES DE LA BITÁCORA</label>
                <textarea 
      ...
      {activeCreator && (
        <div className="p-4 bg-slate-100/50 dark:bg-slate-950/30 border-b border-slate-200 dark:border-white/5 animate-in slide-in-from-top-4 duration-200">
          <TechnicalSurface variant="surface" className="p-4 bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl text-xs flex flex-col gap-3 font-sans">
            <div className="flex justify-between items-center select-none">
              ...
              <button onClick={() => setActiveCreator(null)} className="text-slate-400 hover:text-slate-650 dark:hover:text-slate-200">
              ...
              <div className="flex flex-col gap-3 font-sans">
                ...
                <div className="flex flex-col md:flex-row gap-3 justify-between items-start md:items-center select-none py-1.5 border-t border-slate-200 dark:border-white/5 mt-1">
                  ...
                  <div className="flex flex-col gap-1 w-full md:w-auto">
                    <label className="text-[10px] text-slate-500 dark:text-slate-400 font-bold font-mono uppercase">Categoría de la Nota</label>
                    <div className="flex gap-1.5">
                      {[
                        { id: 'general', label: 'General 📄', colorClass: 'border-slate-400 text-slate-700 bg-slate-500/5 dark:text-slate-350' },
                        { id: 'requirement', label: 'Requerimiento 🛠️', colorClass: 'border-blue-500 text-blue-600 bg-blue-500/5' },
                        { id: 'pain_point', label: 'Dolor ⚠️', colorClass: 'border-amber-500 text-amber-600 bg-amber-500/5' },
                        { id: 'budget', label: 'Presupuesto 💰', colorClass: 'border-emerald-500 text-emerald-600 bg-emerald-500/5' }
                      ].map(pill => {
                  ...
                  <div className="flex items-center gap-2 self-end md:self-center font-sans text-xs">
                    ...
                    />
                    <label htmlFor="pin-note-checkbox" className="text-slate-700 dark:text-slate-300 font-bold select-none cursor-pointer">
                      📌 Fijar en el perfil
                    </label>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-3 font-sans">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 bg-red-500/5 dark:bg-red-500/10 p-3 rounded-xl border border-red-500/20 dark:border-red-500/10 select-none">
                  <div className="flex items-center gap-2">
                    <span className={`w-2.5 h-2.5 rounded-full bg-red-500 ${isTimerRunning ? 'animate-pulse' : ''}`} />
                    <div className="flex flex-col gap-0.5">
                      <h4 className="text-xs font-bold text-red-800 dark:text-red-300">Simulación de Llamada VoIP</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-end border-t border-slate-200 dark:border-white/5 pt-3 select-none">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] text-slate-400 dark:text-slate-500 font-bold font-mono uppercase">Resultado de la Llamada</label>
                    <div className="flex flex-wrap gap-1.5">
                      {[
                        { id: 'Conectado', label: 'Contestó', colorClass: 'border-emerald-500 text-emerald-600 bg-emerald-500/5' },
                        { id: 'Ocupado', label: 'Volver a llamar', colorClass: 'border-amber-500 text-amber-600 bg-amber-500/5' },
                        { id: 'Buzon', label: 'Buzón', colorClass: 'border-indigo-500 text-indigo-600 bg-indigo-500/5' },
                        { id: 'Sin Respuesta', label: 'No Contestó', colorClass: 'border-rose-500 text-rose-600 bg-rose-500/5' }
                      ].map(pill => {
                        ...
                        return (
                          <button
                            key={pill.id}
                            type="button"
                            onClick={() => setNewCall(prev => ({ ...prev, outcome: pill.id }))}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] text-slate-450 dark:text-slate-500 font-bold font-mono uppercase">Plantilla de Minuta</label>
                      <Select 
                        size="sm"
                        onChange={(e) => {
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] text-slate-450 dark:text-slate-500 font-bold font-mono uppercase">Fecha y Hora</label>
                      <Input 
                        type="datetime-local" 
                        value={newCall.date}
                ...
                <div className="flex flex-col gap-1 mt-1">
                  <label className="text-[10px] text-slate-455 dark:text-slate-500 font-bold font-mono uppercase">Resumen de la Llamada</label>
                  <textarea 
              ...
              <div className="flex flex-col gap-3 font-sans">
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex flex-col gap-1 col-span-2 md:col-span-1">
                    <label className="text-[10px] text-slate-500 dark:text-slate-400 font-bold font-mono">TÍTULO DE LA TAREA</label>
                    <Input 
                      type="text" 
                      value={newTask.title}
                    ...
                    <div className="flex flex-wrap gap-1 mt-1.5 select-none">
                      {[
                        ...
                        { label: '📋 Solicitar docs', title: 'Solicitar documentos pendientes' }
                      ].map(pill => (
                        <button
                          key={pill.label}
                          type="button"
                  ...
                  <div className="flex flex-col gap-1 col-span-2 md:col-span-1">
                    <label className="text-[10px] text-slate-500 dark:text-slate-400 font-bold font-mono">FECHA LÍMITE (VENCIMIENTO)</label>
                    <Input 
                      type="datetime-local" 
                    ...
                    <div className="flex flex-wrap gap-1 mt-1.5 select-none">
                      {[
                        ...
                        <button
                          key={pill.label}
                          type="button"
                          onClick={() => setNewTask(prev => ({ ...prev, dueDate: pill.getVal() }))}
                          className="px-2.5 py-1 rounded-xl bg-slate-50 hover:bg-slate-100 dark:bg-slate-900/40 dark:hover:bg-slate-900 text-[10px] text-slate-650 dark:text-slate-400 border border-slate-200 dark:border-white/5 transition-all"
                        >
                          {pill.label}
                        </button>
                ...
                <div className="flex flex-col gap-1.5 mt-1 select-none">
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 font-bold font-mono">PRIORIDAD DE LA TAREA</label>
                  <div className="flex gap-2">
                    {[
                      { id: 'high', label: 'Alta 🔴', colorClass: 'border-rose-500 text-rose-600 bg-rose-500/5' },
                      { id: 'medium', label: 'Media 🟡', colorClass: 'border-amber-500 text-amber-600 bg-amber-500/5' },
                      { id: 'low', label: 'Baja 🔵', colorClass: 'border-blue-500 text-blue-600 bg-blue-500/5' }
                    ].map(pill => {
                ...
                <div className="flex flex-col gap-1 mt-1">
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 font-bold font-mono">DETALLES ADICIONALES (OPCIONAL)</label>
                  <textarea 
            ...
            {activeCreator === 'whatsapp' && (
              <div className="flex flex-col gap-3 font-sans">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 bg-emerald-500/5 dark:bg-emerald-500/10 p-3.5 rounded-xl border border-emerald-500/20 dark:border-emerald-500/10">
                  <div className="flex flex-col gap-0.5">
                    <h4 className="text-xs font-bold text-emerald-800 dark:text-emerald-300">Enlace de Comunicación Directa</h4>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400">
                ...
                <div className="flex flex-col md:flex-row gap-3 items-start md:items-center justify-between border-t border-slate-200 dark:border-white/5 pt-3 select-none">
                  <div className="flex flex-col gap-1 w-full md:w-auto">
                    <label className="text-[10px] text-slate-500 dark:text-slate-400 font-bold font-mono uppercase">Plantillas de Mensaje</label>
                    <Select 
                      size="sm"
                      onChange={(e) => {
                        const template = WHATSAPP_TEMPLATES.find(t => t.id === e.target.value);
                        if (template) {
                  ...
                  <div className="flex flex-col gap-1 items-start md:items-end font-mono text-[9px]">
                    <span className="text-slate-450 dark:text-slate-500 uppercase font-bold">LATENCIA DE CONTACTO</span>
                    <span className={latency.colorClass}>{latency.text}</span>
                  </div>
                </div>
                
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] text-slate-500 dark:text-slate-400 font-bold font-mono uppercase">Pegar Registro de Conversación</label>
                  <textarea 
      ...
      <div className="flex-1 overflow-hidden grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-slate-200 dark:divide-white/5 bg-slate-50/50 dark:bg-slate-950/20">
        ...
        <div className="lg:col-span-1 p-5 overflow-y-auto custom-scrollbar flex flex-col gap-5 bg-slate-100/10 dark:bg-slate-950/10">
          <div className="flex flex-col gap-1.5 border-b border-slate-200 dark:border-white/5 pb-3">
            ...
            <h3 className="text-xs font-bold text-slate-800 dark:text-white uppercase tracking-wider select-none">Perfil de Datos</h3>
          </div>
          <InfoPanel />
                ...
                {lead.activityLog
                  ...
                  .map(entry => {
                    const categoryColors = {
                      general: 'bg-slate-100 text-slate-700 dark:bg-slate-900 dark:text-slate-350 border-slate-200 dark:border-white/5',
                      requirement: 'bg-blue-500/10 text-blue-600 dark:bg-blue-500/15 dark:text-blue-400 border-blue-500/20 dark:border-blue-500/10',
                      pain_point: 'bg-amber-500/10 text-amber-600 dark:bg-amber-500/15 dark:text-amber-400 border-amber-500/20 dark:border-amber-500/10',
                      budget: 'bg-emerald-500/10 text-emerald-600 dark:bg-emerald-500/15 dark:text-emerald-400 border-emerald-500/20 dark:border-emerald-500/10'
                    };
                    const categoryLabel = {
                      general: 'General',
                    ...
                    return (
                      <div 
                        ...
                        className={`p-3 rounded-xl border flex flex-col gap-1.5 bg-white dark:bg-slate-900 ${
                          entry.category === 'requirement' ? 'border-blue-500/20 dark:border-blue-500/10' :
                          entry.category === 'pain_point' ? 'border-amber-500/20 dark:border-amber-500/10' :
                          entry.category === 'budget' ? 'border-emerald-500/20 dark:border-emerald-500/10' :
                          'border-slate-200 dark:border-white/5'
                        }`}
                      >
                        <div className="flex justify-between items-center text-[10px] select-none">
                          <span className={`px-2 py-0.5 rounded-full border text-[9px] font-bold uppercase tracking-wider ${categoryColors[entry.category || 'general']}`}>
