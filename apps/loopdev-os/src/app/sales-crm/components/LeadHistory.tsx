    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end text-xs mt-3 mb-4">
        ...
        >
          <option
            value="all"
            className="bg-surface-light dark:bg-surface-dark text-text-main dark:text-white"
          >
            Todos los estados
          </option>
          <option
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="border-b border-border-technical bg-shell-canvas/60 select-none">
              <th className="p-4 font-bold text-text-muted uppercase tracking-wider text-[10px]">
                Contacto
              </th>
              <th className="p-4 font-bold text-text-muted uppercase tracking-wider text-[10px]">
                Compañía
              ...
              <th className="p-4 font-bold text-text-muted uppercase tracking-wider text-[10px] text-right">
                Acciones
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border-technical/30">
            {paginatedLeads.map((lead) => {
              const isSanitas =
                lead.company.toLowerCase().includes('sanitas') ||
                lead.company.toLowerCase().includes('logística');
              const isStale =
                ...
                >
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2 font-mono font-bold text-primary">
                      ...
                      <StatusPulse
                        variant={
                          lead.aiScore >= 80 ? 'success' : lead.aiScore >= 50 ? 'energy' : 'danger'
                        }
                        size="xs"
