...
export const PipelineCard: React.FC<PipelineCardProps> = ({ lead, onClick, isDragging }) => {
  ...
  const isStale = useMemo(() => {
    // Only 'contacted' leads can be considered stale in the pipeline logic
    ...
    return diffDays > 5;
  }, [lead.stage, lead.lastContactDate]);
    ...
    >
      <div className="flex-grow min-w-0">
        <div className="flex justify-between items-start mb-1.5">
          <h3 className={cn("text-sm font-bold truncate pr-2", isSanitas ? "text-white" : "text-slate-800")}>{lead.name}</h3>
          <div className={cn("text-[10px] font-bold px-1 py-0.5 rounded", lead.aiScore >= 80 ? "bg-emerald-500/20 text-emerald-400" : "bg-rose-500/20 text-rose-400")}>{lead.aiScore}</div>
        </div>
        <div className="flex flex-col gap-0.5">
          <span className={cn("text-[10px] font-bold px-1.5 py-0.5 rounded w-fit", isSanitas ? "bg-white/20" : "bg-blue-50 text-blue-700")}>{displayCompany}</span>
          <p className={cn("text-xs opacity-70", isSanitas ? "text-white" : "text-slate-500")}>{lead.interestedPlan || 'Más Salud'}</p>
        </div>
      </div>

      <div className={cn("mt-4 mb-2 flex items-center gap-2", isSanitas ? "text-white/40" : "text-slate-200")}>
        ...
        <div className="flex items-center gap-1.5 select-none opacity-80">
          <Icon name="vital_signs" size="sm" className={cn("w-4 h-3", isSanitas ? "text-white" : "text-slate-400")} />
          <span className={cn("text-[8px] font-black uppercase tracking-[0.2em]", isSanitas ? "text-white" : "text-slate-400")}>Actividad</span>
        </div>
        <div className={cn("h-px flex-1", isSanitas ? "bg-white/20" : "bg-slate-100")}></div>
      </div>
      ...
      <div className="flex items-end justify-between">
        <div className="flex flex-col min-w-0">
          <span className={cn("text-[10px] font-bold", isSanitas ? "text-white" : "text-slate-700")}>{actorName}</span>
          <span className={cn("text-[10px] opacity-50", isSanitas ? "text-white" : "text-slate-400")}>{formattedDate}</span>
        </div>
