...
interface LeadDetailContextType {
  ...
  setNewCall: React.Dispatch<React.SetStateAction<{ outcome: string; summary: string; date: string }>>;
  newTask: { title: string; description: string; dueDate: string; priority: 'high' | 'medium' | 'low' };
  setNewTask: React.Dispatch<React.SetStateAction<{ title: string; description: string; dueDate: string; priority: 'high' | 'medium' | 'low' }>>;
...
export const LeadDetailProvider: React.FC<LeadDetailProviderProps> = ({ children, initialLead, onClose }) => {
  const { updateLead } = useSalesCrm();
  const [editedLead, setEditedLead] = useState<Lead>(initialLead);
  const [isEditing, setIsEditing] = useState(false);
  
  // Creators state
  const [activeCreator, setActiveCreator] = useState<'note' | 'call' | 'task' | 'whatsapp' | 'email' | null>(null);
  const [activeDetailCard, setActiveDetailCard] = useState<'contact' | 'product' | 'source' | 'value' | null>(null);
  const [newNote, setNewNote] = useState('');
  const [noteCategory, setNoteCategory] = useState<'general' | 'requirement' | 'pain_point' | 'budget'>('general');
  const [notePinned, setNotePinned] = useState(false);
  const [newCall, setNewCall] = useState({ outcome: 'Conectado', summary: '', date: new Date().toISOString().slice(0, 16) });
  const [newTask, setNewTask] = useState({ title: '', description: '', dueDate: new Date().toISOString().slice(0, 16), priority: 'medium' as 'high' | 'medium' | 'low' });
  ...
  const handleEditChange = useCallback((e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    setEditedLead(prev => {
      ...
      if (name === 'dealValue' || name === 'winProbability' || name === 'height' || name === 'weight' || name === 'annualPremium' || name === 'landingVisits') {
  ...
  const addNote = useCallback(() => {
    ...
    const updatedLead: Lead = {
      ...initialLead,
      activityLog: [logEntry, ...(initialLead.activityLog || [])],
      history: [
        ...initialLead.history,
        {
  ...
  const toggleTask = useCallback((taskToToggle: LeadTask) => {
    const nextStatus = taskToToggle.status === 'Completed' ? 'Pending' : 'Completed';
    const logEntry: ActivityLogEntry = {
      timestamp: new Date().toISOString(),
      actor: 'Elena Gómez',
      type: nextStatus === 'Completed' ? 'TASK_COMPLETED' : 'TASK_CREATED',
      action: nextStatus === 'Completed' ? `Tarea completada: ${taskToToggle.title}` : `Tarea reactivada: ${taskToToggle.title}`
    };
    const updatedLead: Lead = {
      ...initialLead,
      tasks: (initialLead.tasks || []).map(t => t.id === taskToToggle.id ? {
        ...t,
        status: nextStatus,
      ...
      history: [
        ...
        {
          date: new Date().toISOString().split('T')[0],
          action: nextStatus === 'Completed' ? `Tarea completada: ${taskToToggle.title}` : `Tarea reabierta: ${taskToToggle.title}`,
          actor: 'Elena Gómez (Sales)'
        }
      ]
    };
  ...
  const handleDocumentUpload = useCallback((file: File, category: string) => {
  ...
  const emailSent = useCallback((subject: string, body: string, attachmentsCount: number) => {
    const logEntry: ActivityLogEntry = {
      timestamp: new Date().toISOString(),
      actor: 'Elena Gómez',
      type: 'GENERIC',
  ...
  const handleLeadClosure = useCallback((action: 'rejected' | 'discarded', reason: string, details: string) => {
    const logEntry: ActivityLogEntry = {
      timestamp: new Date().toISOString(),
      actor: 'Elena Gómez',
      type: 'STATUS_CHANGE',
  ...
  const handleStatusChange = useCallback((newStage: Lead['stage']) => {
    const logEntry: ActivityLogEntry = {
      timestamp: new Date().toISOString(),
      actor: 'Elena Gómez',
      type: 'STATUS_CHANGE',
  ...
  return (
    <LeadDetailContext.Provider value={{
      lead: initialLead,
      editedLead,
      isEditing,
      isReadOnly,
