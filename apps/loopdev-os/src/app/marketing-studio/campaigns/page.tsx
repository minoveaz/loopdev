...
export default function MarketingCampaignsPage() {
  ...
  async function handleCreate(event: React.FormEvent<HTMLFormElement>) {
    ...
    setObjective('');
    setIsCreating(false);
    await queryClient.invalidateQueries({ queryKey: ['marketing-campaigns', activeOrganizationId, marketingWorkspace.id] });
  }
  ...
  return (
    <main className="mx-auto flex h-full max-w-7xl flex-col p-8 lg:p-10">
      ...
      {canManage && (
        <div className="mb-8">
          {!isCreating ? (
            <button type="button" onClick={() => setIsCreating(true)} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary/90">
              New Campaign
            </button>
          ) : (
            <form onSubmit={handleCreate} className="max-w-2xl rounded-xl border border-border-subtle bg-surface-elevated p-6">
              <Heading size="sm">Create campaign</Heading>
              <div className="mt-5 grid gap-4">
                <label className="grid gap-2 text-sm">
                  <span className="text-text-muted">Brand</span>
                  <select value={brandId} onChange={(event) => setBrandId(event.target.value)} className="rounded-lg border border-border-subtle bg-surface px-3 py-2">
                    {brands.map((brand) => <option key={brand.id} value={brand.id}>{brand.name}</option>)}
                  </select>
                </label>
                <label className="grid gap-2 text-sm">
                  <span className="text-text-muted">Name</span>
                  <input required minLength={1} maxLength={160} value={name} onChange={(event) => setName(event.target.value)} className="rounded-lg border border-border-subtle bg-surface px-3 py-2" />
                </label>
                <label className="grid gap-2 text-sm">
                  <span className="text-text-muted">Objective</span>
                  <textarea required minLength={1} maxLength={240} value={objective} onChange={(event) => setObjective(event.target.value)} className="min-h-24 rounded-lg border border-border-subtle bg-surface px-3 py-2" />
                </label>
                ...
                <div className="flex gap-3">
                  ...
                  <button type="button" onClick={() => setIsCreating(false)} className="rounded-lg border border-border-subtle px-4 py-2 text-sm">Cancel</button>
                </div>
              </div>
            </form>
          )}
