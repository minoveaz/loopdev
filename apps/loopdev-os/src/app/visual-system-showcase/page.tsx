...
import { Button, EmptyState, Heading, LoadingState, Skeleton, TechnicalSurface } from '@loopdev/ui';
  ...
  return (
    <section className="border-border-technical bg-background rounded-lg border p-5">
      ...
      <div className="mt-6 space-y-5">
        <div>
          <span className="text-text-muted text-xs">Display / page title</span>
          <p className="text-text-main text-3xl font-semibold">Operational overview</p>
        </div>
        <div>
          <span className="text-text-muted text-xs">Heading / section</span>
          <p className="text-text-main text-xl font-semibold">Recent activity</p>
        </div>
        <div>
          <span className="text-text-muted text-xs">Body / supporting</span>
          <p className="text-text-muted text-sm">
            Use semantic type tokens and verify expansion with translated content.
          </p>
        </div>
        <div>
          <span className="text-text-muted text-xs">Caption / metadata</span>
          <p className="text-text-muted font-mono text-[10px] uppercase tracking-[0.16em]">
            Updated today · ready
          </p>
        </div>
        <div>
          <span className="text-text-muted text-xs">JetBrains Mono / technical</span>
          <p className="text-text-main font-mono text-sm">
            module.crm.contacts · 2026-08-14T16:20Z
          </p>
          <p className="text-text-muted mt-1 font-mono text-[10px]">
            tenant_id=acme_042 · trace=7f31c8
          </p>
        </div>
      </div>
      <Checklist
        items={[
          'Font family is token-backed',
          'Weight communicates hierarchy',
          'Line-height remains readable',
          'Long labels do not clip',
          'Labels work in mobile widths',
        ]}
      />
    </section>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="border-border-technical bg-shell-canvas rounded-lg border p-5">
        <Heading as="h2" size="sm" weight="bold" className="text-text-main">
          Nesting and contrast combinations
        </Heading>
        <p className="text-text-muted mt-1 text-sm">
          Review hierarchy, readable text and borders through the real surface stack.
        </p>
        <div className="mt-4 grid gap-4 lg:grid-cols-3">
          <TechnicalSurface
            ...
            className="p-5"
          >
            <span className="text-text-muted text-xs">canvas → surface → elevated</span>
            <TechnicalSurface variant="surface" radius="md" border="subtle" className="mt-4 p-4">
              <TechnicalSurface
                variant="surface"
                depth="raised"
                radius="lg"
                ...
                className="p-4"
              >
                <strong className="text-text-main">Readable elevated content</strong>
                <p className="text-text-muted mt-1 text-xs">Text and border remain distinct.</p>
              </TechnicalSurface>
            </TechnicalSurface>
          </TechnicalSurface>
          <TechnicalSurface
            ...
            withGrid
          >
            <span className="text-text-muted text-xs">canvas → surface → overlay</span>
            <TechnicalSurface variant="surface" radius="md" border="subtle" className="mt-4 p-4">
              <TechnicalSurface
                variant="surface"
                depth="overlay"
                ...
                className="p-4"
              >
                <strong className="text-text-main">Overlay surface</strong>
                <p className="text-text-muted mt-1 text-xs">Does not obscure context.</p>
              </TechnicalSurface>
            </TechnicalSurface>
          </TechnicalSurface>
          <TechnicalSurface
            ...
            withGrid
          >
            <span className="text-text-muted text-xs">canvas → glass → surface</span>
            <TechnicalSurface
              variant="glass"
              depth="raised"
              radius="lg"
            ...
            >
              <TechnicalSurface variant="surface" radius="md" border="subtle" className="p-4">
                <strong className="text-text-main">Glass treatment</strong>
                <p className="text-text-muted mt-1 text-xs">
                  Requires contrast and performance evidence.
                </p>
              </TechnicalSurface>
            </TechnicalSurface>
          </TechnicalSurface>
...
function StatesView({
  ...
  > = {
    ready: {
      ...
      message: 'Summary, metrics and activity are ready to review.',
      action: 'View details',
      tone: 'border-brand-cyan/50 bg-brand-cyan/10',
    },
    loading: {
      title: 'Loading workspace',
      message: 'Keep the final geometry stable while content is loading.',
      action: 'Cancel',
      tone: 'border-primary/50 bg-primary/10',
    },
    empty: {
      title: 'Nothing here yet',
      message: 'Explain what the user can create and offer one clear next step.',
      action: 'Create first item',
      tone: 'border-accent/50 bg-accent/10',
    },
    error: {
      title: 'Unable to load',
      message: 'Explain the failure and provide a recovery action.',
      action: 'Retry',
      tone: 'border-danger/50 bg-danger/10',
    },
    forbidden: {
      title: 'Access restricted',
      message: 'Differentiate permission failure from an empty result.',
      action: 'Request access',
      tone: 'border-danger/50 bg-danger/10',
    },
    'read-only': {
      title: 'Read-only view',
      message: 'Keep content visible while mutation actions are unavailable.',
      action: 'View policy',
      tone: 'border-border-technical bg-surface-elevated',
    },
    offline: {
      title: 'Offline mode',
      message:
        'Connection is unavailable. Keep cached data visible and make synchronization status explicit.',
    ...
    stale: {
      ...
      message:
        'Connection is available, but this content needs a freshness check before it is trusted.',
      action: 'Refresh data',
      tone: 'border-accent/50 bg-accent/10',
    },
    conflict: {
      title: 'Conflict detected',
      message: 'Show the conflict and provide a resolution path.',
      ...
      <section className={`border-2 rounded-lg p-6 ${activeState.tone}`}>
        {isEmptyState ? (
          <EmptyState
            ...
            action={
              <Button
                variant={state === 'empty' ? 'primary' : state === 'error' ? 'danger' : 'outline'}
                size="sm"
          ...
          <>
            ...
            <div className="mt-5">
              ...
              <p className="text-text-muted mt-1 text-sm">{activeState.message}</p>
              <Button
                variant={state === 'conflict' ? 'danger' : 'outline'}
                size="sm"
                className="mt-4"
                onClick={() =>
                  onStateChange(state === 'conflict' || state === 'loading' ? 'ready' : state)
                }
              >
                {activeState.action}
              </Button>
  ...
  const viewportPreviews = [
    {
      ...
      sidebar: 'w-20',
      panel: 'w-28',
      note: 'Full shell with persistent navigation and context panel',
    },
    {
      label: 'Tablet 1024',
      width: 'max-w-2xl',
      layout: 'grid-cols-[auto_1fr]',
      sidebar: 'w-14',
      panel: 'hidden',
      note: 'Compact navigation; context panel becomes an explicit action',
    },
    {
      label: 'Mobile 390',
      width: 'max-w-xs',
      ...
      sidebar: 'hidden',
      panel: 'hidden',
      note: 'Single canvas column; navigation and panels become touch surfaces',
    },
  ] as const;
  ...
  return (
    <div className="space-y-4">
      ...
      {viewportPreviews.map(({ label: viewport, width, layout, sidebar, panel, note }) => (
        <article
          key={viewport}
          className="border-border-technical bg-background rounded-lg border p-4"
        >
          <div className="flex items-center justify-between gap-3">
            <Heading as="h2" size="sm" weight="bold" className="text-text-main">
              {viewport}
            </Heading>
            <span className="text-accent text-xs">Review target</span>
          </div>
          ...
          >
            ...
            <div className={`mt-2 grid min-h-48 gap-2 ${layout}`}>
              <div
                className={`border-border-technical bg-surface-dark min-w-0 rounded-md border p-2 ${sidebar}`}
              >
                <span className="text-text-muted block max-w-full break-words text-[9px] leading-tight">
                  SuiteSidebar
                </span>
                <div className="mt-3 space-y-2">
                  <span className="bg-primary/70 block h-2 rounded" />
                  <span className="bg-surface-elevated block h-2 rounded" />
                  <span className="bg-surface-elevated block h-2 rounded" />
                </div>
              </div>
              <div className="border-border-technical bg-surface-light dark:bg-surface-dark rounded-md border p-2">
                <span className="text-text-muted text-[9px]">SuiteCanvas</span>
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <span className="bg-primary/30 block h-14 rounded" />
                  <span className="bg-surface-elevated block h-14 rounded" />
                  <span className="bg-accent/30 col-span-2 block h-8 rounded" />
                </div>
                <div className="bg-surface-elevated mt-2 h-5 rounded" />
              </div>
              <div
                className={`border-border-technical bg-surface-elevated rounded-md border p-2 ${panel}`}
              >
                <span className="text-text-muted text-[9px]">ContextPanel</span>
                <div className="bg-background mt-3 h-16 rounded" />
              </div>
            </div>
            <div className="border-border-technical bg-surface-elevated mt-2 h-5 rounded-md border px-2">
              <span className="text-text-muted text-[9px]">Footer / status</span>
            </div>
          </div>
          <a
            href="/composition-showcase?recipe=SuiteOverview"
            title={`Open SuiteOverview for ${viewport}`}
            className="text-accent hover:text-primary mt-3 inline-block text-xs font-semibold"
          >
            Open real SuiteOverview at this viewport
          </a>
          <Checklist
            items={[
              'No horizontal page overflow',
              'Sidebar transformation is explicit',
              'Panel becomes drawer/overlay',
              'Touch targets remain usable',
              'Timeline/table scroll is internal',
            ]}
          />
        </article>
      ))}
...
export default function VisualSystemShowcasePage() {
  ...
  const content = {
    ...
    States: <StatesView state={state} onStateChange={setState} />,
    Responsive: <ResponsiveView />,
  }[tab];

  useEffect(() => {
    const root = document.documentElement;
    root.dataset.loopdevShowcaseTheme = theme;
    return () => {
      delete root.dataset.loopdevShowcaseTheme;
    };
  }, [theme]);
        ...
        <header className="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-text-muted font-mono text-xs uppercase tracking-[0.18em]">
              LoopDev visual certification
            </p>
            <Heading as="h1" size="3xl" weight="bold" className="mt-2">
              SaaS visual system showcase
            </Heading>
            <p className="text-text-muted mt-2 max-w-3xl text-sm">
              Review each fixture before composing CRM screens. This page shows what to inspect and
              where the contract applies.
            </p>
          </div>
          <div
            className="border-border-technical bg-background flex shrink-0 gap-1 rounded-md border p-1"
