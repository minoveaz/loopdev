...
export async function POST(request: Request) {
  const parsed = CreateCommunicationInternalNoteCommandSchema.safeParse(
  ...
  try {
    return NextResponse.json(
      await createInternalNote({ ...parsed.data, authorId: access.userId }),
      { status: 201 },
    );
  } catch {
    return NextResponse.json({ error: 'Unable to create communication note' }, { status: 500 });
  }
}
