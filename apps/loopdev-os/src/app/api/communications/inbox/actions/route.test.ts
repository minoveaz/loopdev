...
const { authorizeCommunications, assignConversationToSelf, changeConversationStatus, createInternalNote, sendWhatsAppConversationTemplate, sendWhatsAppConversationText } = vi.hoisted(() => ({
...
describe('Communications inbox actions API', () => {
  ...
  it('uses the reply permission and delegates reply execution to Core', async () => {
    ...
    const response = await POST(request({ organizationId, conversationId, action: 'reply', body: 'Hello' }));
    expect(response.status).toBe(200);
    expect(authorizeCommunications).toHaveBeenCalledWith(organizationId, 'communications.reply');
    expect(sendWhatsAppConversationText).toHaveBeenCalledWith({ organizationId, conversationId, body: 'Hello' });
  });

  it('uses the note permission and passes the authorized actor to Core', async () => {
    createInternalNote.mockResolvedValue({ id: 'note-1' });
    const response = await POST(request({ organizationId, conversationId, action: 'note', body: 'Follow up tomorrow' }));
    expect(response.status).toBe(201);
    expect(authorizeCommunications).toHaveBeenCalledWith(organizationId, 'communications.note');
    expect(createInternalNote).toHaveBeenCalledWith({ organizationId, conversationId, body: 'Follow up tomorrow', authorId: userId });
  });
  ...
  it('routes assignment and lifecycle through their own permissions', async () => {
    ...
    const assignResponse = await POST(request({ organizationId, conversationId, action: 'assign' }));
    const statusResponse = await POST(request({ organizationId, conversationId, action: 'status', status: 'closed' }));

    expect(assignResponse.status).toBe(200);
    expect(statusResponse.status).toBe(200);
    expect(authorizeCommunications).toHaveBeenNthCalledWith(1, organizationId, 'communications.assign');
    expect(authorizeCommunications).toHaveBeenNthCalledWith(2, organizationId, 'communications.lifecycle');
    expect(assignConversationToSelf).toHaveBeenCalledWith({ organizationId, conversationId, userId });
    expect(changeConversationStatus).toHaveBeenCalledWith({ organizationId, conversationId, status: 'closed' });
  });
