...
export async function GET(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      {
        error: 'Invalid CRM opportunity query',
...
export async function POST(request: Request) {
  ...
  if (!body || typeof body !== 'object')
    return NextResponse.json(
      { error: 'Invalid CRM opportunity command', code: 'VALIDATION_ERROR' },
      { status: 400 },
    );
  ...
  if ('leadId' in body && 'productKey' in body && !('contactId' in body)) {
    ...
    if (!access.allowed)
      return NextResponse.json({ error: 'Unauthorized' }, { status: access.status });
    try {
      const result = await createOpportunityFromLead(conversion.data, access.userId);
  ...
  if (!manual.success)
    return NextResponse.json(
      {
        error: 'Invalid manual CRM opportunity command',
        code: 'VALIDATION_ERROR',
        details: manual.error.flatten(),
      },
      { status: 400 },
    );
  ...
  if (!access.allowed)
    return NextResponse.json({ error: 'Unauthorized' }, { status: access.status });
  try {
    const result = await createManualOpportunity(manual.data, access.userId);
    return NextResponse.json(result.opportunity, { status: result.created ? 201 : 200 });
  } catch (error) {
