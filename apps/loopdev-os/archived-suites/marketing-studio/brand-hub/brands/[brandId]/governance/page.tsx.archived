    ...
    <div className="flex flex-col gap-8">
      <header className="flex flex-col gap-2">
        ...
        <LpdText size="sm" className="text-text-muted max-w-xl">
          This is the governance view for the active brand.
        </LpdText>
      </header>
      <div className="rounded-2xl border border-border-technical p-6">
        <LpdText size="sm" className="text-text-muted">
          {isLoading ? 'Loading governance context…' : context?.rules.engine ? 'Governance rules are available in the published brand context.' : 'No governance rules configured yet.'}
        </LpdText>
      </div>
    </div>
  );
