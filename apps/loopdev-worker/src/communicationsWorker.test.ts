...
describe('Communications worker', () => {
  it('acknowledges a valid job and emits a redacted success log', async () => {
    ...
    expect(log).toHaveBeenCalledWith(
      expect.objectContaining({ event: 'job_succeeded', traceId: 'trace-1' }),
    );
  ...
  it('marks invalid payloads as validation failures without dispatching a handler', async () => {
    const queue = {
      receive: vi.fn().mockResolvedValue({ id: job.id, type: 'delivery' }),
      acknowledge: vi.fn(),
      fail: vi.fn(),
    };
    const handlers = { deliver: vi.fn(), retry: vi.fn(), purge: vi.fn() };
  ...
  it('records handler failures with a normalized code', async () => {
    const queue = { receive: vi.fn().mockResolvedValue(job), acknowledge: vi.fn(), fail: vi.fn() };
    const handlers = {
      deliver: vi.fn().mockRejectedValue(new WorkerJobError('PROVIDER_UNAVAILABLE', 'offline')),
      retry: vi.fn(),
      purge: vi.fn(),
    };
    const log = vi.fn();
    ...
    expect(log).toHaveBeenCalledWith(
      expect.objectContaining({ event: 'job_failed', errorCode: 'PROVIDER_UNAVAILABLE' }),
    );
  });
  ...
  it('selects only expired records without legal hold for a purge dry run', () => {
    ...
    expect(
      isRetentionEligible(
        { id: 'held', lastActivityAt: '2024-01-01T00:00:00.000Z', legalHold: true },
        now,
      ),
    ).toBe(false);
    expect(
    ...
    ).toEqual({ eligibleIds: ['expired'], dryRun: true });
  });
});
