...
select ok(exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'organization_memberships' and column_name = 'status'), 'memberships expose a lifecycle status');
...
select ok(position('membership.status = ''active''' in pg_get_functiondef('public.has_organization_permission(uuid,text)'::regprocedure)) > 0, 'permission helper requires an active membership');
