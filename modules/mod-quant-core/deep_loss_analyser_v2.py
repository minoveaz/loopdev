...
async def deep_analyze_losses():
    ...
    for exit_ev in res.data:
        ...
        sig = res_sig.data[0] if res_sig.data else {}
        # Intentar encontrar el tipo de señal en diferentes posibles nombres de columna
        sig_type = sig.get('signal_type', sig.get('type', sig.get('metadata', {}).get('reason', 'UNKNOWN')))
