...
test('shell showcase remains within the project viewport without horizontal overflow', async ({
  page,
}) => {
  const viewport = page.viewportSize();
  const viewportName =
    viewport?.width && viewport.width <= 320
      ? 'mobile-compact'
      : viewport?.width && viewport.width < 768
        ? 'mobile'
        : viewport?.width === 1024
          ? 'tablet'
          : 'desktop';
  ...
  expect(
    ...
    `shell overflows horizontally at ${viewportName}`,
...
test('iPad breakpoint keeps the suite navigation toggle visible beside the logo', async ({
  page,
}) => {
  ...
  const geometry = await toggle.evaluate((element) => {
    ...
    return {
      isInsideHeader: Boolean(
        headerRect && toggleRect.top >= headerRect.top && toggleRect.bottom <= headerRect.bottom,
      ),
