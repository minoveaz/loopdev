...
import {
  CrmActivityReadSchema,
  CrmActivitySchema,
  CrmAuditEventSchema,
  CrmCaptureLeadCommandSchema,
  CrmContactConsentSchema,
  CrmCompanySchema,
  CrmContactSchema,
  CrmCreateLeadCommandSchema,
  CrmEntityLookupPageSchema,
  CrmLeadSchema,
  CrmLeadAttributionSchema,
  CrmNoteReadSchema,
  CrmNoteSchema,
  CrmRelatedPersonSchema,
  CrmTaskSchema,
} from '../crm';
...
const ids = {
  ...
  leadId: '00000000-0000-4000-9000-000000000003',
...
describe('CRM contracts', () => {
  ...
  it('keeps related people distinct from contactable contacts', () => {
    const related = CrmRelatedPersonSchema.parse({
      id: ids.id,
      organizationId: ids.organizationId,
      contactId: ids.contactId,
      firstName: 'Lucía',
  ...
  it('requires channel consent and a source for lead commands', () => {
    expect(
      CrmContactConsentSchema.safeParse({
        id: ids.id,
        organizationId: ids.organizationId,
        contactId: ids.contactId,
        channel: 'whatsapp',
    ...
    expect(
      CrmCreateLeadCommandSchema.safeParse({
        organizationId: ids.organizationId,
        contactId: ids.contactId,
        source: manualSource,
      }).success,
  ...
  it('enforces shared activity deduplication and append-only shape', () => {
    ...
    expect(
      CrmActivityReadSchema.safeParse({ ...activity, sourceKey: `task:${ids.leadId}` }).success,
    ).toBe(false);
    expect(
      CrmActivityReadSchema.safeParse({ ...activity, details: 'private details' }).success,
    ).toBe(false);
  });
  ...
  it('redacts unauthorized note bodies and bounds lookup pages', () => {
    ...
    expect(
      CrmEntityLookupPageSchema.safeParse({ items: [], nextCursor: null, hasMore: false }).success,
    ).toBe(true);
  });
  ...
  it('preserves source attribution for captured leads', () => {
    ...
    expect(
      CrmLeadAttributionSchema.safeParse({
        id: ids.id,
        organizationId: ids.organizationId,
        leadId: ids.leadId,
        source: 'campaign',
