...
export const CommunicationTemplateStatusSchema = z.enum([
  'draft',
  'approved',
  'rejected',
  'archived',
]);
export const CommunicationAccountOnboardingStatusSchema = z.enum([
  'pending',
  'connected',
  'failed',
  'expired',
...
export const CommunicationWorkerJobSchema = z
  .object({
    id: IdSchema,
  ...
  .superRefine((value, context) => {
    if (
      (value.type === 'delivery' || value.type === 'retry') &&
      (!value.accountId || !value.messageId)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'delivery and retry jobs require accountId and messageId',
      });
    }
    if (value.type === 'purge' && (value.accountId || value.messageId)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'purge jobs must not target an account or message',
      });
    }
  });
...
export type StartWhatsAppEmbeddedSignupCommand = z.infer<
  typeof StartWhatsAppEmbeddedSignupCommandSchema
>;
...
export type CompleteWhatsAppEmbeddedSignupCommand = z.infer<
  typeof CompleteWhatsAppEmbeddedSignupCommandSchema
>;
...
export type SynchronizeCommunicationTemplatesCommand = z.infer<
  typeof SynchronizeCommunicationTemplatesCommandSchema
>;
...
export const SendCommunicationCommandSchema = z
  .object({
    organizationId: IdSchema,
    conversationId: IdSchema,
    channel: CommunicationChannelSchema,
...
export type CreateCommunicationConversationCommand = z.infer<
  typeof CreateCommunicationConversationCommandSchema
>;

export const CreateCommunicationMessageCommandSchema = z
  .object({
    organizationId: IdSchema,
    conversationId: IdSchema,
    externalId: z.string().trim().max(240).nullable().optional(),
...
export type CreateCommunicationMessageCommand = z.infer<
  typeof CreateCommunicationMessageCommandSchema
>;
...
export type CreateCommunicationInternalNoteCommand = z.infer<
  typeof CreateCommunicationInternalNoteCommandSchema
>;
...
export type RecordCommunicationMessageStatusCommand = z.infer<
  typeof RecordCommunicationMessageStatusCommandSchema
>;
...
export type RetryCommunicationMessageCommand = z.infer<
  typeof RetryCommunicationMessageCommandSchema
>;
