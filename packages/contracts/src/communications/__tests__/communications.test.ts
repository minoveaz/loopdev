...
import { CommunicationConversationSchema, CommunicationPermissionKeySchema, CommunicationTemplateSchema, CommunicationWorkerJobSchema, SendCommunicationCommandSchema, StartWhatsAppEmbeddedSignupCommandSchema } from '../communications';
...
describe('Communications Core contracts', () => {
  ...
  it('requires a long opaque state for WhatsApp Embedded Signup', () => {
    expect(StartWhatsAppEmbeddedSignupCommandSchema.safeParse({ organizationId: ids.organizationId, state: 'too-short' }).success).toBe(false);
    expect(StartWhatsAppEmbeddedSignupCommandSchema.safeParse({ organizationId: ids.organizationId, state: 'a'.repeat(32) }).success).toBe(true);
  });

  it('models an approved account-scoped WhatsApp template', () => {
    expect(CommunicationTemplateSchema.safeParse({ id: ids.contactId, organizationId: ids.organizationId, accountId: ids.organizationId, channel: 'whatsapp', externalTemplateId: 'template-1', name: 'welcome', language: 'es', category: 'utility', status: 'approved', body: 'Hola {{1}}', parameterNames: ['first_name'], createdAt: timestamp, updatedAt: timestamp }).success).toBe(true);
  });

  it('requires scoped identifiers for durable delivery and retry jobs', () => {
    expect(CommunicationWorkerJobSchema.safeParse({ id: ids.conversationId, type: 'delivery', organizationId: ids.organizationId, traceId: 'trace-1', createdAt: timestamp }).success).toBe(false);
    expect(CommunicationWorkerJobSchema.safeParse({ id: ids.conversationId, type: 'retry', organizationId: ids.organizationId, accountId: ids.contactId, messageId: ids.contactId, traceId: 'trace-1', createdAt: timestamp }).success).toBe(true);
    expect(CommunicationWorkerJobSchema.safeParse({ id: ids.conversationId, type: 'purge', organizationId: ids.organizationId, traceId: 'trace-1', createdAt: timestamp }).success).toBe(true);
  });
});
